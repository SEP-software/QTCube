...
void MainWindow::create_panels() {
  ...
  for (int i = 0; i < nviews; i++) {
    ...
    int npanx = pars->getInt(std::string(par.toLatin1().constData()), 1);
    par = tr("npanely") + QString::number(i + 1);
    int npany = pars->getInt(std::string(par.toLatin1().constData()), 1);
    int npan = npanx * npany;
    for (int ipan = 0; ipan < npan; ipan++, inum++) {
      std::shared_ptr<panel> x(new panel(my_pan->get_next_panel_num(), pars,
                                         pos, pk, datas, 0, colort, cur_func,
                                         serv, my_or, my_maps));
      my_pan->add_panel(x);
    }
  }
...
MainWindow::MainWindow(std::shared_ptr<genericIO> ios,
                       ...
                       std::shared_ptr<pick_draw> _pks,
                       std::shared_ptr<slice_types> col) {
  _io = ios;
  ...
  connect(datas.get(), SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));

  // setStyleSheet("border-radius 0px; font: bold 14px; padding: 0px;
  // border-style: inset; spacing: 0px"); setStyleSheet("QGroupBox { spacing:
  // 40px }");
  setStyleSheet("margin: 3px");
  setStyleSheet("QGroupBox { border: 2px solid gray; border-radius: 3px; }");
  setStyleSheet(
      "QGroupBox::title { background-color: transparent; subcontrol-position: "
      "top left; padding:9 13px;}");
  pk = _pks;
  stat_view = "mouse";
  this->colort = col;
  ...
  std::shared_ptr<window_panel> myw22(
      new window_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  my_window_panel = myw22;
  my_window_panel->setSizePolicy(QSizePolicy::Expanding,
                                 QSizePolicy::Expanding);
  my_window_panel->setMaximumHeight(75);

  std::shared_ptr<panel_panel> pp22(
      new panel_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  my_panel_panel = pp22;
  ...
  std::shared_ptr<data_panel> myp(
      new data_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  ...
  std::shared_ptr<color_panel> cp2(
      new color_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  my_color_panel = cp2;
  std::shared_ptr<help_panel> hp2(
      new help_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  std::shared_ptr<ano_panel> ap2(
      new ano_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  std::shared_ptr<pick_panel> pp3(
      new pick_panel(pos, my_wind, my_pan, datas, pk, colort, my_maps));
  my_help_panel = hp2;
...
void MainWindow::connect_it() {
  connect(my_pan.get(), SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
  connect(my_panel_panel.get(), SIGNAL(actionDetected(std::vector<QString>)),
          this, SLOT(actionRespond(std::vector<QString>)));
  connect(my_pan.get(), SIGNAL(windows_update()), my_wind.get(),
          SLOT(update_all()));
  connect(my_window_panel.get(), SIGNAL(actionDetected(std::vector<QString>)),
          this, SLOT(actionRespond(std::vector<QString>)));
  connect(my_wind.get(), SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
  connect(my_hist.get(), SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
  connect(my_ano_panel.get(), SIGNAL(actionDetected(std::vector<QString>)),
          this, SLOT(actionRespond(std::vector<QString>)));
  connect(my_color_panel.get(), SIGNAL(actionDetected(std::vector<QString>)),
          this, SLOT(actionRespond(std::vector<QString>)));
  connect(my_pick_panel.get(), SIGNAL(actionDetected(std::vector<QString>)),
          this, SLOT(actionRespond(std::vector<QString>)));
  connect(my_data_panel.get(), SIGNAL(actionDetected(std::vector<QString>)),
          this, SLOT(actionRespond(std::vector<QString>)));
}
...
void MainWindow::actionRespond(std::vector<QString> coms) {
  ...
  if (coms[0].contains("auto"))
    coms.insert(coms.begin(), QString::number(my_wind->get_active_num()));
  if (!coms[1].contains("clip")) my_hist->addEvent(coms);

  if (coms[1].contains("navigate"))
    my_pan->perform_navigate(coms);
  else if (coms[1].contains("pick"))
    my_pan->perform_pick(coms);
  else if (coms[1].contains("auto"))
    my_pan->perform_auto(coms);
  else if (coms[1].contains("display")) {
    my_pan->perform_display(coms);
  } else if (coms[1].contains("navigate"))
    my_pan->perform_navigate(coms);
  else if (coms[1].contains("orient"))
    my_pan->perform_orient(coms);
  else if (coms[1].contains("windows"))
    my_wind->windows_respond(coms);
  else if (coms[1].contains("annotate"))
    my_wind->annotate_respond(coms);

  else if (coms[1].contains("view"))
    my_pan->perform_view(coms);
  else if (coms[1].contains("ctable"))
    colort->update_colortable(coms);
  else if (coms[1].contains("data")) {
    datas->update_data(coms);
    my_wind->update_displays(true);
  } else if (coms[1].contains("redraw")) {
    if (coms[2].contains("only"))
      my_pan->update_only(coms[0].toInt());
    else if (coms[2].contains("all"))
      my_pan->update_all();
    else if (coms[2].contains("limited"))
      my_pan->update_it(coms[0].toInt());
  }
  if (!coms[1].contains("panels") && !coms[1].contains("windows") &&
      !coms[1].contains("menu") && !coms[1].contains("clip")) {
    if (coms[1].contains("ctable"))
      my_wind->update_displays(true);
    else
      my_wind->update_displays();
  }
  ...
  bool v;
  int i = coms[0].toInt(&v);

  if (v) {
    if (i != -1 && !coms[1].contains("menu") && !coms[1].contains("clip")) {
      if (coms[1].contains("windows") || coms[1].contains("annotate") ||
          coms[1].contains("data")) {
...
void MainWindow::update_status_bar() {
  if (stat_view.contains("mouse") > 0) {
    ...
    std::shared_ptr<mouse_overlay> x =
        std::static_pointer_cast<mouse_overlay>(funcs["overlay"]);
    ...
    if (what == DISPLAY_EXTRA)
      dis = "Display: Extra";
    else if (what == DISPLAY_POINTS)
      dis = "Display: Points";
    else if (what == DISPLAY_LINE)
      dis = "Display: Line";
    ...
    modeLabel->setText(color + " " + drawing + " " + ex + " " + dist + " " +
                       dis + " " + move);
...
void MainWindow::lockViews() { my_pan->set_locked(true); }
void MainWindow::unlockViews() { my_pan->set_locked(false); }
...
void MainWindow::openAnno() { my_ano_panel->show(); }
void MainWindow::closeData() {}
void MainWindow::openData() { my_data_panel->show(); }
void MainWindow::closeColor() {}
void MainWindow::openColor() { my_color_panel->show(); }
void MainWindow::closePick() {}
void MainWindow::openPick() { my_pick_panel->show(); }
void MainWindow::closeHelp() {}
void MainWindow::openHelp() { my_help_panel->show(); }
void MainWindow::about() {
  QMessageBox::about(
      this, tr("About Sep_cube"),
      tr("<b>Sep_cube</b>  is a multi-dimensional viewer of regularly sampled "
         "cube.  "
