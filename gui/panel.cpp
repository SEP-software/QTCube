...
panel::panel(int inum, std::shared_ptr<paramObj> pr,
             std::shared_ptr<position> p, std::shared_ptr<pick_draw> _pks,
             std::shared_ptr<datasets> ds, int idata,
             std::shared_ptr<slice_types> ct, std::shared_ptr<mouse_func> f,
             std::shared_ptr<orientation_server> server,
             std::shared_ptr<orients> oo, std::shared_ptr<maps> mym) {
  ...
  this->myv = view_by_name(pars->getString(
      std::string("view") + std::to_string(iview), std::string("THREE")));
  // Sets the initial data color to grey.
  std::string col =
      pars->getString(std::string("color") + std::to_string(iview), "grey");
  ...
  for (int i = 0; i < 7; i++) com.push_back("");

  // Setup the possible views
  view_types x = view_types();
  std::map<QString, bool> *myl = x.return_views();
  for (std::map<QString, bool>::iterator i = myl->begin(); i != myl->end(); i++)
    allv.push_back(i->first);
...
std::shared_ptr<panel> panel::clone(int in) {
  std::shared_ptr<panel> x(new panel(in, pars, pos, pk, datas, idat, colort,
                                     func, serv, my_or, my_maps));
  return x;
}
...
void panel::set_o_data(int id) {
  ...
  dat_o = datas->return_dat(id);
  idat_o = id;
  myv = view_by_name(view_name);
  myv_o = view_by_name(view_name);

  fact_o->set_bcolor(QString::fromStdString(dat_o->get_bcolor()));
...
void panel::set_data(int id) {
  dat = datas->return_dat(id);
  idat = id;

  std::vector<QString> com;
  com.push_back(QString::number(idat));
  com.push_back("menu");

  com.push_back("Clip");

  com.push_back("set_pts");
  com.push_back(QString::fromStdString(dat->return_clip()));
  ...
  QString junk = QString::fromStdString(dat->return_histogram());
  std::vector<QString> com2;
  com2.push_back(QString::number(idat));
  com2.push_back("menu");
  com2.push_back("Clip");
  ...
  fact->set_bcolor(QString::fromStdString(dat->get_bcolor()));
  fact->set_ecolor(QString::fromStdString(dat->get_ecolor()));
  com2[2] = "Orient";
  ...
  int ns[8];
  for (int i = 0; i < 8; i++) ns[i] = ax[i].n;
  com2[3] = QString::fromStdString(util::string_from_int_array(8, ns));

  emit actionDetected(com2);

  QString valid =
      QString::fromStdString(dat->validate_view(view_name.toStdString(), pos));
  // if(valid!=view_name
  update_view(valid);
}
...
void panel::update_view(QString name) {
  ...
  bool set = false;
  name = QString::fromStdString(dat->validate_view(name.toStdString(), pos));
...
std::shared_ptr<view> panel::view_by_name(const QString &nameIn) {
  ...
  QString name = nameIn;
  if (this->dat->getRestrict() != "NONE") {
    name = QString::fromStdString(this->dat->getRestrict());
  }

  name = QString::fromStdString(dat->validate_view(name.toStdString(), pos));
  view_name = name;
  // The FRONT, SIDE and TOP views all display one single face so the single
  // class is called
  if (name == "FRONT") {
    std::shared_ptr<single> myfront(
        new single(iview_s, 0, 1, fm, pk, draw_o, proportions));
    my = myfront;
  } else if (name == "SIDE") {
    std::shared_ptr<single> myside(
        new single(iview_s, 0, 2, fm, pk, draw_o, proportions));
    my = myside;
  } else if (name == "TOP") {
    std::shared_ptr<single> mytop(
        new single(iview_s, 1, 2, fm, pk, draw_o, proportions));
    my = mytop;
  }
  // CUT, CUBE and THREE all call their own individual classes since each is a
  // unique type of view.
  else if (name == "CUT") {
    std::shared_ptr<cube_cut> mycut(
        new cube_cut(iview_s, fm, pk, draw_o, proportions));
    my = mycut;
  } else if (name == "CUBE") {
    std::shared_ptr<cube> mycube(
        new cube(iview_s, fm, pk, draw_o, proportions));
    my = mycube;
  } else if (name == "MULTI") {
    std::shared_ptr<multi_face> mymf(
        new multi_face(iview_s, fm, pk, draw_o, grid1, grid2, jplane));
    my = mymf;
  } else {
    std::shared_ptr<three_face> myt(
        new three_face(iview_s, fm, pk, draw_o, proportions));
    my = myt;
  }
...
int panel::add_clip_pt(QString str) {
  int iret = dat->add_pt(str.section(":", 0, 0).toFloat(),
                         str.section(":", 1, 1).toFloat());
  std::vector<QString> com;
  com.push_back(iview_s);
  com.push_back("menu");
  com.push_back("Clip");
...
void panel::get_view_pcts(float *pt, float *pf) { myv->get_pcts(pt, pf); }
...
void panel::draw_base_pixmap(QPainter *painter) {
  ...
  if (overlay) {
    myv->viewit(painter, &pen, fact, dat->_data, pos, begx, endx, begy, endy,
                drawit, false);
    myv_o->viewit(painter, &pen, fact_o, dat_o->_data, pos, begx, endx, begy,
                  endy, drawit, true);
  } else {
    this->myv->viewit(painter, &pen, fact, dat->_data, pos, begx, endx, begy,
                      endy, drawit, false);
  }
...
void panel::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (timeit.elapsed() > 200) {
    if (mouse_down == 2) {
      ...
      actionDetected(myv->right_mouse_move(pctx, pcty, pos, func));
    timeit.restart();
  }
}
