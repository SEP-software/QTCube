...
void windows::annotate_respond(std::vector<QString> coms) {
  ...
  if (coms[2].contains("shape")) {
    ...
    int *vv = util::int_array_from_string(coms[3].toStdString());
...
void windows::windows_respond(std::vector<QString> coms) {
  ...
  if (coms[2].contains("save_big")) {
    ...
    update_grid(active, active->return_grid1(), active->return_grid2(),
                coms[3].toInt(), active->return_grid2());
    active->update_display(true);
  } else if (coms[2].contains("split_y")) {
    update_grid(active, active->return_grid1(), active->return_grid2(),
                active->return_grid1(), coms[3].toInt());
    active->update_display(true);
...
void windows::update_grid(std::shared_ptr<DrawWindow> active, int noldx,
                          int noldy, int nnewx, int nnewy) {
  std::vector<QString> coms;
  coms.push_back("-1");
  coms.push_back("panels");
  coms.push_back("delete");
  coms.push_back("xx");
...
void windows::update_all() { update_displays(); }
...
void windows::connect_it(int i) {
  connect(my_winds[i].get(), SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
}
void windows::disconnect_it(int i) {
  disconnect(my_winds[i].get(), SIGNAL(actionDetected(std::vector<QString>)),
