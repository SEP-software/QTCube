...
using namespace SEP;
window_panel::window_panel(std::shared_ptr<position> p,
  ...
  for (int i = 0; i < my_w->nwind(); i++) {
    my_tabs[QString::number(i + 1)] = new sub_window_panel(
        p, my_w, my_p, my_d, pk, c, mym, QString::number(i));
    addTab(my_tabs[QString::number(i + 1)], QString::number(i + 1));
    sub_window_panel *x = (sub_window_panel *)my_tabs[QString::number(i + 1)];
    connect(x, SIGNAL(actionDetected(std::vector<QString>)), this,
  ...
  if (coms[1].contains("windows")) {
    ...
    if (coms[2].contains("add")) {
      my_tabs[QString::number(ip)] =
          new sub_window_panel(my_pos, my_wind, my_pan, my_dat, my_pick,
                               my_slices, my_maps, QString::number(inum));
      addTab(my_tabs[QString::number(ip)], QString::number(ip));
      connect(my_tabs[QString::number(ip)],
              SIGNAL(actionDetected(std::vector<QString>)), this,
              SLOT(actionRespond(std::vector<QString>)));
    }
  }
}
sub_window_panel::sub_window_panel(std::shared_ptr<position> p,
                                   std::shared_ptr<windows> my_w,
                                   std::shared_ptr<panels> my_p,
                                   std::shared_ptr<datasets> my_d,
                                   std::shared_ptr<pick_draw> pk,
                                   std::shared_ptr<slice_types> c,
                                   std::shared_ptr<maps> my_m, QString n) {
  ...
  splitXBox =
      new basicComboBox("Split X", "Number of sections to split in X", options);
  splitYBox =
      new basicComboBox("Split Y", "Number of sections to split in X", options);
  options.clear();
  for (int i = 0; i < 11; i++) options.push_back(QString::number(i));
  boundaryBox = new basicComboBox(
      "Boundary", "Number of pixels to make the boundary", options);
  ...
  connect(splitXBox->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(split_x(const QString &)));
  connect(splitYBox->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(split_y(const QString &)));
  connect(boundaryBox->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(boundary_change(const QString &)));
