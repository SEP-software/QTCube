...
class panel : public QWidget {
 ...
 public:
  panel(int inum, std::shared_ptr<SEP::paramObj> pars,
        std::shared_ptr<position> pos, std::shared_ptr<pick_draw> _pks,
        std::shared_ptr<datasets> ds, int idat, std::shared_ptr<slice_types> ct,
 ...
 public:
  ...
  void set_proportions(float *props) {
    for (int i = 0; i < 8; i++) proportions[i] = props[i];
  }
  ...
  void set_no_map() { pos->set_no_shift(); }
  void delete_overlay();
  ...
  void return_grid_loc(float *p, int *iloc) {
    pos->return_grid_loc(p, iloc, oversamp);
  }
  int get_order(int i) { return this->pos->get_order(i); }

  // Panel size functions
  void get_panel_dimensions(int *bx, int *by, int *ex, int *ey) {
    *bx = begx;
    *by = begy;
    *ex = endx;
    *ey = endy;
  }
  void set_panel_dimensions(int bx, int by, int ex, int ey) {
    begx = bx;
    begy = by;
    endx = ex;
    endy = ey;
  ...
  std::shared_ptr<Qdataset> return_dat() { return dat; }
  void set_data(int id);
  int get_idat() { return idat; }
  void iter_data(int inc);

  // Grid for multi-plot
  void set_jplane(int j) {
    jplane = j;
 ...
 private:
  ...
  std::shared_ptr<Qdataset>
      dat = nullptr,
      dat_o = nullptr;  // Current dataset as primary and overlaid
