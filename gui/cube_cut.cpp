...
#include <ctime>
using namespace SEP;
cube_cut::cube_cut(QString view, std::shared_ptr<QFontMetrics> f_m,
                   std::shared_ptr<pick_draw> _pks,
                   std::shared_ptr<draw_other> dr, std::vector<float> prop) {
  pct_top = .4;
  pct_front = .6;
  fm = f_m;
  set_basics(_pks, dr);
...
void cube_cut::viewit(QPainter *painter, QPen *pen, std::shared_ptr<slice> fact,
                      std::shared_ptr<dataset> dat,
                      std::shared_ptr<orient_cube> pos, int b_x, int e_x,
                      int b_y, int e_y, std::shared_ptr<draw_what> draws,
                      bool overlay) {
  cur_dat = dat;
  ...
  if (proportions[0] < -1)
    ...
    get_corner_loc(&cx, &cy, proportions[pos->get_order(0)],
                   proportions[pos->get_order(1)],
  ...
  slices[0]->set_draw_params(fm, ori, 0, 1, 0, front_x_b, front_x_e, front_y_b,
                             front_y_e, draws->draw_pos, false,
                             draws->draw_axis1, draws->draw_axis2, false, 0.,
                             0., false, false);

  slices[1]->set_draw_params(fm, ori, 2, 1, 0, top_x_b, top_x_e, top_y_b,
                             top_y_e, draws->draw_pos, false, false,
                             draws->draw_axis3, false, 0., slope, true, false);
  slices[2]->set_draw_params(
      fm, ori, 0, 2, 0, side_x_b, side_x_e, side_y_b, side_y_e, draws->draw_pos,
      false, draws->draw_axis3, false, false, 1. / slope, 0., false, false);
  ...
  top_x_b = (int)((pct[1] - .01) * ((float)(top_x_e - top_x_b - baxes)) +
                  baxes - shift + baxes * -1. / slope);
  ...
  slices[3]->set_draw_params(fm, ori_t, 2, 1, 0, top_x_b + shiftx,
                             shiftx + top_x_e, shifty + top_y_b,
                             shifty + top_y_e, false, false, false, false,
                             false, 0., slope, true, false, true);
  ...
  front_y_e =
      (int)(pct[0] * (float)(front_y_e - front_y_b - baxes) + front_y_b);
  front_x_b = (int)(pct[1] * (float)(front_x_e - front_x_b - baxes) +
                    front_x_b + baxes);

  shifty = (int)((pct[2]) * (float)(cy - by - bound_t));
  shiftx = (int)(shifty * slope);

  slices[4]->set_draw_params(fm, ori_f, 0, 1, 0, front_x_b - shiftx,
                             front_x_e - shiftx, front_y_b - shifty,
                             front_y_e - shifty, false, false, false, false,
  ...
  slices[5]->set_draw_params(fm, ori_s, 0, 2, 0, side_x_b + shiftx,
                             shiftx + side_x_e, shifty + side_y_b,
                             shifty + side_y_e, false, false, false, false,
                             false, 1. / slope, 0., false, false);
  ...
  */
  //    if(draws->draw_pos) for(int i=0; i < 3; i++) this->pg->draw(painter,
  //    pos, slices[i],dat);
  if (draws->draw_pos)
    for (int i = 0; i < 3; i++) this->pk->draw(painter, slices[i]);
