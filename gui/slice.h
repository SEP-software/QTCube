...
public:
  ...
  void set_draw_params(std::shared_ptr<QFontMetrics> fm,
                       ...
                       int off, int bx, int ex, int by, int ey, int dopos,
                       bool at, bool ab, bool al, bool ar, float sh1, float sh2,
                       bool rev1, bool rev2, bool verb = false);

  void update_pos(std::vector<QString> *com, int ix, int iy,
                  std::shared_ptr<orient_cube> pos);
  void set_overlay() { is_overlay = true; }
  bool in_slice(int ix, int iy);
  void update_range(std::vector<QString> *com, int ix_old, int iy_old, int ix,
                    int iy, std::shared_ptr<orient_cube> pos);
  ...
  void get_image_pos(float x, float y, int *id1, int *id2,
                     bool ignore_rev = false);
  void get_shifted_image_pos_from_sam_fract(float ix, float iy, int *id1,
                                            int *id2);
  ...
  void get_shifted_image_pos_from_loc(float fx, float fy, int *ix, int *iy,
                                      bool ignore_rev = false);
  void get_image_pos_pct(float fx, float fy, int *ix, int *iy);
  void get_slice_axes(std::shared_ptr<orient_cube> pos, int *i1, int *i2,
                      int *i3);
  ...
  virtual void draw_slice(QPainter *painter, std::shared_ptr<dataset> dat,
                          QPen *pen, std::shared_ptr<orient_cube> pos, bool ov,
                          bool draw_grid) {
    if (painter == 0 && !dat && pen == 0 && !pos && ov && draw_grid) {
      ;
    };
  }
  std::vector<float> optimal_with_font(int n, float min, float max);
  std::vector<float> optimal_with_font(int n, int mn, int mx,
                                       std::vector<float> values,
                                       std::vector<float> *tic_vals);
  ...
  std::shared_ptr<pairs_new> return_pick_locs(QString col,
  ...
  int optimal_scale_linear(int nsz, float min, float max, float *onum,
                           float *dnum);
  void draw_axes(QPainter *painter, std::shared_ptr<orient_cube> pos,
                 bool draw_grid);
