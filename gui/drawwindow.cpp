...
DrawWindow::DrawWindow(int ng1, int ng2,
                       std::vector<std::shared_ptr<panel>> pan, int in) {
  ngrid1 = ng1;
  ngrid2 = ng2;  // Grid size of panels to display
  inum = in;
  for (int i = 0; i < (int)pan.size(); i++) {
    panels.push_back(pan[i]);
  }
...
void DrawWindow::pdf_save(QString sm, QString file) {
  ...
  int rangex = printer->pageRect().width(),
      rangey = printer->pageRect().height();
...
void DrawWindow::shift_finish_anno(int bx, int by, int ex, int ey) {
  ano->shift_finish_annotate(
      (float)bx / (float)width(), (float)by / (float)height(),
      (float)ex / (float)width(), (float)ey / (float)height());
}
...
void DrawWindow::resizeEvent(QResizeEvent *x) {
  ...
  if (width() != pmScaled.width() ||
      height() != pmScaled.height()) {  // if new size,
...
void DrawWindow::update_panel_map() {
  ...
  float sm = 0;
  for (int i = 0; i < ngrid1; i++) sm += port1[i];
  for (int i = 0; i < ngrid1; i++) {
    port1[i] /= sm;
  }
  sm = 0;
  for (int i = 0; i < ngrid2; i++) sm += port2[i];
  for (int i = 0; i < ngrid2; i++) port2[i] /= sm;

  beg1[0] = 0;
  end1[0] = (int)(port1[0] * W);
  for (int i = 1; i < ngrid1; i++) {
    beg1[i] = end1[i - 1] + bound_thick;
    end1[i] = beg1[i] + (int)(Wd * port1[i]);
  }

  beg2[0] = 0;
  end2[0] = (int)(port2[0] * H);
  for (int i = 1; i < ngrid2; i++) {
    beg2[i] = end2[i - 1] + bound_thick;
    end2[i] = beg2[i] + (int)(Ht * port2[i]);
  }
      ...
      if (i < mouse_boundx) {
        if (bariers[i] >= bariers[mouse_boundx]) {
          if (i == 0)
            ...
            bariers[i] = bariers[i - 1];
        }
      } else if (i > mouse_boundx) {
        if (bariers[i] <= bariers[mouse_boundx])
          bariers[i] = bariers[mouse_boundx];
      }
    }
    port1[0] = bariers[0];
    for (int i = 1; i < ngrid1; i++) port1[i] = bariers[i] - bariers[i - 1];
    delete[] bariers;
  }
  if (mouse_boundy != -1) {
    ...
    for (int i = 0; i < ngrid2; i++) {
      if (i < mouse_boundy) {
        ...
        if (bariers[i] <= bariers[mouse_boundy])
          bariers[i] = bariers[mouse_boundy];
      }
    }
    port2[0] = bariers[0];
    for (int i = 1; i < ngrid2; i++) port2[i] = bariers[i] - bariers[i - 1];
    delete[] bariers;
  }
  update_panel_map();
  draw_base_pixmap();
...
void DrawWindow::mousePressEvent(QMouseEvent *e) {
  ...
  if (mouse_panel >= 0)
    panels[mouse_panel]->mousePressEvent(e);
  else {
    timeit.start();
  }
}
void DrawWindow::mouseDoubleClickEvent(QMouseEvent *e) {
  mouse_panel = get_panel(e->x(), e->y());
  if (mouse_panel >= 0)
    panels[mouse_panel]->mouseDoubleClickEvent(e);
  else if (e->button() == Qt::LeftButton) {
    for (int i = 0; i < ngrid1; i++) {
      port1[i] = 1. / (float)ngrid1;
    }
    for (int i = 0; i < ngrid2; i++) {
      port2[i] = 1. / (float)ngrid2;
    }
  }
  draw_base_pixmap();
}
