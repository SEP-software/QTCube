...
class data_panel : public qcubeTabBasic {
  Q_OBJECT
 public:
  data_panel(std::shared_ptr<SEP::position> p,
             std::shared_ptr<SEP::windows> my_w,
             std::shared_ptr<SEP::panels> my_p,
             std::shared_ptr<SEP::datasets> my_d, std::shared_ptr<pick_draw> pk,
             std::shared_ptr<SEP::slice_types> c,
...
class add_dataset : public qcubeTabBasic {
  Q_OBJECT
 public:
  add_dataset(std::shared_ptr<position> p, std::shared_ptr<SEP::windows> my_w,
              std::shared_ptr<SEP::panels> my_p,
              std::shared_ptr<SEP::datasets> my_d,
 ...
 private:
  ...
  void delete_sep_tab();
  QWidget *sepTab, *seispakTab, *nmoTab, *semblanceTab, *flattenTab,
      *surfaceTab;
  QHBoxLayout *sepLay, *seispakLay, *nmoLay, *semblanceLay, *flattenLay,
      *surfaceLay;
  basicRadioBox *storeSEP, *storeSEIS, *esizeSEIS, *typeSURF;
  basicButtonBox *loadSEP, *loadSEIS, *createSurface, *createFlatten,
      *surfUpdate;
  ;
  basicComboBox *colorFlatten, *dataSurface, *colorSurface, *dataOffset,
      *dataMeas;
...
class data_sub_panel : public qcubeTabBasic {
 ...
 public:
  data_sub_panel(std::shared_ptr<position> p, std::shared_ptr<windows> my_w,
                 ...
                 std::shared_ptr<maps> mym, int idat);
