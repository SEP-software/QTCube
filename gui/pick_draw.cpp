...
pick_draw::pick_draw() {
  ...
  for (int i = 0; i < (int)cvec.size(); i++) {
    std::shared_ptr<pick_param> p(new pick_param(
        0, DISPLAY_POINTS, 2, myc->return_qcolor(cvec[i]), font_name));
    all[cvec[i]] = p;
  }
}
...
void pick_draw::increase_size() {
  if (draw_what == "all") {
    ...
    all.find(active_col)->second->increase_size();
}
void pick_draw::decrease_size() {
  if (draw_what == "all") {
    ...
    all.find(active_col)->second->decrease_size();
}
void pick_draw::set_single(int ex) {
  if (draw_what == "all") {
    ...
    all.find(active_col)->second->set_single(ex);
}
...
void pick_draw::set_display(int ex) {
  if (draw_what == "all") {
    ...
    all.find(active_col)->second->set_display(ex);
}
void pick_draw::delete_pick_draw() { buck.reset(); }
...
void pick_draw::float_to_int(const float *floc, int *iloc) {
  for (int i = 0; i < 8; i++) {
    if (d[i] == 0.)
      ...
      iloc[i] =
          std::max(0, std::min(n[i] - 1, (int)((floc[i] - o[i]) / d[i] + .5)));
  }
}
void pick_draw::read_file(const QString &text) {
  ...
  if ((fd = fopen(text.toLatin1().constData(), "r")) == NULL)
    fprintf(stderr, "PROBLEM OPENING \n");
  int i = 0;
  while (!feof(fd)) {
    fgets(line, 1000, fd);
    if (0 != strncmp(line, "#", 1)) {
      sscanf(line, "%s %f %f %f %f %f %f %f %f %d %d %s", color, &floc[0],
             &floc[1], &floc[2], &floc[3], &floc[4], &floc[5], &floc[6],
             &floc[7], &type, &extra, txt);
      col = color;
...
void pick_draw::write_file(const QString &text) {
  FILE *fd;
  float floc[8];
  fd = fopen(text.toLatin1().constData(), "w");
  for (int i = 0; i < buck->get_nbuckets(); i++) {
    ...
    for (int j = 0; j < b->return_size(); j++) {
      ...
      fprintf(fd, "%s %f %f %f %f %f %f %f %f %d %d %s\n", p->col.c_str(),
              floc[0], floc[1], floc[2], floc[3], floc[4], floc[5], floc[6],
              floc[7], p->type, p->extra, p->txt.c_str());
    }
  }
  fclose(fd);
}
void pick_draw::add_rotated_pt(int orient_num, int *iloc, int type, QString col,
                               int ex, QString t) {
  long long pos = buck->loc_to_index(iloc);
  std::shared_ptr<pick_new> pk(new pick_new(
      iloc, pos, 1, active_col.toStdString(), ex, t.toStdString()));
...
void pick_draw::add_pt(int *iloc, int type, QString col, int ex, QString txt) {
  ...
  long long pos = buck->loc_to_index(iloc);
  std::shared_ptr<pick_new> pk = buck->get_pick(pos, col.toStdString());
  if (pk) {
    for (std::map<int, std::shared_ptr<pick_orient>>::iterator p =
             orient_planes.begin();
         p != orient_planes.end(); p++) {
      std::shared_ptr<pick_new> pk2 = serv->convert_pick(p->second->oc, pk);
      std::shared_ptr<pick_new> pk3 =
          p->second->buck->get_pick(pk2->pos, col.toStdString());
      assert(pk3 != 0);
      p->second->planes->del_pick(pk3);
      p->second->buck->del_pick(pk3->pos, col.toStdString());
    }
    planes->del_pick(pk);
    buck->del_pick(pk->pos, col.toStdString());
  }
  pk = buck->add_pick(iloc, pos, type, col.toStdString(), extra,
                      active_txt.toStdString());
  for (std::map<int, std::shared_ptr<pick_orient>>::iterator p =
           orient_planes.begin();
       p != orient_planes.end(); p++) {
    std::shared_ptr<pick_new> pk2 = serv->convert_pick(p->second->oc, pk);

    std::shared_ptr<pick_new> pk3 = p->second->buck->add_pick(
        pk2->iloc, pk2->pos, pk2->type, pk2->col, pk2->extra, pk2->txt);
...
void pick_draw::delete_multi_picks(std::vector<long long> locs) {
  ...
  for (int i = 0; i < (int)locs.size(); i++) {
    ...
    pk = buck->get_pick(locs[i], col.toStdString());
    if (pk != 0) {
      del_pick_orients(pk);
      planes->del_pick(pk);
      buck->del_pick(locs[i], col.toStdString());
    }
  }
}
void pick_draw::del_nearest(std::shared_ptr<orient_cube> pos, int *iloc,
                            int iax1, int iax2) {
  QString col = active_col;
  if (pos == 0) {
    ;
  ...
  hsh = buck->find_nearest(hsh, iax1, iax2, col.toStdString());

  std::shared_ptr<pick_new> pk = buck->get_pick(hsh, col.toStdString());
  if (!pk) {
    return;
  }
  del_pick_orients(pk);
  planes->del_pick(pk);
  buck->del_pick(hsh, col.toStdString());
}
void pick_draw::del_pt(std::shared_ptr<orient_cube> pos, int *iloc, int iax1,
                       int iax2) {
  QString col = active_col;
  if (pos == 0) {
    ;
...
void pick_draw::del_pick_orients(std::shared_ptr<pick_new> pk) {
  for (std::map<int, std::shared_ptr<pick_orient>>::iterator p =
    ...
    std::shared_ptr<pick_new> pk2 = serv->convert_pick(p->second->oc, pk);
    std::shared_ptr<pick_new> pk3 =
        p->second->buck->get_pick(pk2->pos, pk2->col);
    assert(pk3 != 0);
    p->second->planes->del_pick(pk3);
    p->second->buck->del_pick(pk3->pos, pk3->col);
  }
}
std::shared_ptr<pairs_new> pick_draw::get_pts_sort(
    std::shared_ptr<orient_cube> pos, int *iloc, int iax1, int iax2,
    QString col) {
...
void pick_draw::check_create_oc(std::shared_ptr<orient_cube> pos) {
  ...
  if (orient_planes.count(orient_num)) {
    for (std::list<int>::iterator i = orients.begin(); i != orients.end();
         i++) {
      if (*i == orient_num) orients.erase(i);
      orients.push_front(orient_num);
    }
    return;
  }
  ...
  for (int i = 0; i < nw->return_size(); i++) {
    ...
    dumb = orient_planes[orient_num]->buck->add_pick(p->iloc, p->pos, p->type,
                                                     p->col, p->extra, p->txt);
  }
}
std::shared_ptr<picks_vec> pick_draw::create_load_plane(
    std::shared_ptr<orient_cube> pos, int iax1, int iax2, int *iloc) {
  ...
  int loc[8];
  pos->get_viewed_locs(iax1, iax2, loc);
  loc[iax1] = loc[iax2] = -1;
  check_create_oc(pos);
  if (!orient_planes[oc]->planes->map_exists(loc)) {
    ...
    all = orient_planes[oc]->planes->return_picks(loc);
  return all;
}
std::shared_ptr<picks_vec> pick_draw::return_all_picks(
    std::shared_ptr<orient_cube> pos, QString col) {
  int orient_num = -1;
  if (pos != 0) orient_num = pos->get_orient_num();
  if (orient_num == -1) return buck->return_all_picks(col.toStdString());
  check_create_oc(pos);
  return orient_planes[orient_num]->buck->return_all_picks(col.toStdString());
}
std::shared_ptr<picks_vec> pick_draw::return_iloc_based(int *iloc,
                                                        QString col) {
  std::shared_ptr<picks_vec> all = buck->return_all_picks(col.toStdString());

  std::shared_ptr<picks_vec> sub(new picks_vec());
  for (int i = 0; i < all->return_size(); i++) {
    std::shared_ptr<pick_new> p = all->return_pick(i);
    if (p->col == col.toStdString())
...
void pick_draw::delete_picks(std::shared_ptr<orient_cube> pos, int ival,
                             QString col) {
...
void pick_draw::update_slice_maps(int *iloc, int *inew, int iax1, int iax2,
                                  int iax3, std::shared_ptr<slice> slc) {
  if (iax1 == 0 && iax2 == 0) {
    ;
  };
  ...
  for (int i = std::max(0, iloc[iax3] - dist);
       i <= std::min(n3 - 1, iloc[iax3] + dist); i++) {
    inew[iax3] = i;
  ...
  std::map<int, std::shared_ptr<picks_vec>> all_picks =
      orient_planes[oc]->buck->return_picks_from_range(iax3, new_planes, inew);
...
void pick_param::draw(QPainter *painter, int iax1, int iax2,
                      std::shared_ptr<pairs_new> myp, float rat) {
  bool draw_pt = true;
  ...
  if (draw_pt)
    draw_points(painter, myp, txt);
  else
    draw_line(painter, myp, rat);
}
void pick_param::draw_points(QPainter *painter,
                             std::shared_ptr<pairs_new> points, bool txt) {
  ...
  for (int i = 0; i < (int)points->size(); i++) {
    ...
    ix = points->vals[i].x;
    if (txt) {
      int pw = fm->width(QString::fromStdString(points->vals[i].txt));
      painter->drawText(ix - pw / 2, iy,
                        QString::fromStdString(points->vals[i].txt));
    } else {
      bx = ix - sz;
      by = iy - sz;
      ex = ix + sz;
...
void pick_param::draw_line(QPainter *painter, std::shared_ptr<pairs_new> points,
                           float rat) {
...
void pick_draw::delete_pick_vals(int ival, QString col) {
  std::shared_ptr<picks_vec> pks = buck->get_pick_type(ival, col.toStdString());

  for (int i = 0; i < pks->return_size(); i++) {
    std::shared_ptr<pick_new> pik = pks->return_pick(i);
    del_pick_orients(pik);
    planes->del_pick(pik);
    buck->del_pick(pik->pos, col.toStdString());
  }
}
void pick_draw::clear_picks(std::shared_ptr<orient_cube> pos, int *iloc,
                            QString col) {
  std::shared_ptr<picks_vec> pka =
      orient_planes[pos->get_orient_num()]->planes->return_picks(iloc);
...
std::shared_ptr<pairs_new> pick_draw::get_pts_sort_le(
    std::shared_ptr<orient_cube> pos, int iax1, int iax2, int ival, int isort,
    QString col) {
  int iloc[8];
  pos->get_locs(iloc);
  iloc[iax1] = iloc[iax2] = -1;
  std::shared_ptr<picks_vec> pks = create_load_plane(pos, iax1, iax2, iloc);
  std::shared_ptr<pairs_new> prs(new pairs_new());
  for (int i = 0; i < pks->return_size(); i++) {
    std::shared_ptr<pick_new> p = pks->return_pick(i);
    if ((p->col == col.toStdString()) && p->type <= ival) {
      if (iax1 != isort)
        prs->add_point(p->iloc[iax1], p->iloc[iax2]);
      else
        prs->add_point(p->iloc[iax2], p->iloc[iax1]);
    }
  }
  // if(iax1==isort) {
