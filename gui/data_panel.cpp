...
using namespace SEP;
data_panel::data_panel(std::shared_ptr<position> p,
                       std::shared_ptr<windows> my_w,
                       std::shared_ptr<panels> my_p,
                       std::shared_ptr<datasets> my_d,
                       std::shared_ptr<pick_draw> pk,
                       std::shared_ptr<slice_types> c,
                       std::shared_ptr<maps> mym) {
  set_basics(p, my_w, my_p, my_d, pk, c, mym);

  for (int i = 0; i < my_dat->return_ndat(); i++) {
    QString tit =
        QString::fromStdString(my_dat->return_dat(i)->get_title_short());
    my_tabs[tit] = new data_sub_panel(p, my_w, my_p, my_d, pk, c, mym, i);
    connect(my_tabs[tit], SIGNAL(actionDetected(std::vector<QString>)), this,
            SLOT(actionRespond(std::vector<QString>)));
    addTab(my_tabs[tit], tit);
  }

  my_tabs["add"] = new add_dataset(p, my_w, my_p, my_d, pk, c, mym);
  connect(my_tabs["add"], SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
  addTab(my_tabs["add"], "+");
}
add_dataset::add_dataset(std::shared_ptr<position> p,
                         std::shared_ptr<windows> my_w,
                         std::shared_ptr<panels> my_p,
                         std::shared_ptr<datasets> my_d,
                         std::shared_ptr<pick_draw> pk,
...
void add_dataset::create_sep() {
  ...
  if (!filename.isNull()) {
    ...
    if (storeSEP->what_checked() == 0)
      coms.push_back("BYTE");
    else
      coms.push_back("FLOAT");
...
void add_dataset::create_seispak() {
  ...
  if (!filename.isNull()) {
    ...
    coms.push_back(filename);
    if (storeSEIS->what_checked() == 0)
      coms.push_back("BYTE");
    else
      coms.push_back("FLOAT");
    if (esizeSEIS->what_checked() == 0)
      coms.push_back("SHORT");
    else
      coms.push_back("FLOAT");
    emit actionDetected(coms);
  }
...
void add_dataset::create_surface() {
  ...
  if (typeSURF->what_checked() == 0)
    coms.push_back("amplitude");
  else
...
void add_dataset::create_sep_tab() {
  ...
  storeSEP =
      new basicRadioBox("Storage", "Byte", "Store internally as a float file");
...
void add_dataset::create_seispak_tab() {
  ...
  connect(loadSEIS->my_radios[0], SIGNAL(clicked()), this,
          SLOT(create_seispak()));
...
void add_dataset::create_surface_tab() {
  ...
  for (int i = 0; i < my_dat->return_ndat(); i++)
    dlist.push_back(QString::fromStdString(my_dat->return_dat(i)->get_name()));
  std::vector<QString> mp = my_c->return_cvec();

  colorSurface = new basicComboBox("Pickset", "Pickset to use for surface", mp);
  dataSurface =
      new basicComboBox("Dataset", "Dataset to use for surface", dlist);
  createSurface =
      new basicButtonBox("", "Create", "Create surface datasest", false);
  connect(createSurface->my_radios[0], SIGNAL(clicked()), this,
          SLOT(create_surface()));
  typeSURF =
      new basicRadioBox("TYPE", "amplitude", "Make dataset out of amplitudes");
...
void data_panel::update_menu(std::vector<QString> coms) {
  ...
  if (coms[1].contains("data")) {
    if (coms[2].contains("new")) {
      ...
      QString tit =
          QString::fromStdString(my_dat->return_dat(i)->get_title_short());
      my_tabs[tit] = new data_sub_panel(my_pos, my_wind, my_pan, my_dat,
                                        my_pick, my_slices, my_maps, i);
      connect(my_tabs[tit], SIGNAL(actionDetected(std::vector<QString>)), this,
              SLOT(actionRespond(std::vector<QString>)));
    ...
    if (idat != -1) {
      QString tg =
          QString::fromStdString(my_dat->return_dat(idat)->get_title_short());
      ((data_sub_panel *)my_tabs[tg])->update_menu(coms);
    }
  } else {
    for (std::map<QString, QWidget *>::iterator i = my_tabs.begin();
         i != my_tabs.end(); i++) {
...
void data_sub_panel::update_menu(std::vector<QString> coms) {
  ...
  char temp_ch[1024];
  sprintf(temp_ch, "%f",
          my_dat->return_dat(idat)->get_value(
              my_pan->get_panel(0)->return_orient()));

  val->setText(1, tr(temp_ch));

/*
  std::vector<std::string> hS = my_dat->return_dat(idat)->get_header_vals(
      my_pan->get_panel(0)->return_orient());
  for (int ih = 0; ih < (int)hS.size(); ih++) {
    headers[ih]->setText(1, QString::fromStdString(hS[ih]));
  }
  */
#ifdef CHEVRON
  ...
  if (dat_type.contains("FLATTEN")) {
    int myv = (int)my_dat->return_dat(idat)->get_value(
        my_pan->get_panel(0)->return_orient());
    flatChoose->set_checked(flat->button_from_value(myv));
  }
#endif
}
data_sub_panel::data_sub_panel(std::shared_ptr<position> p,
                               std::shared_ptr<windows> my_w,
                               std::shared_ptr<panels> my_p,
                               std::shared_ptr<datasets> my_d,
                               std::shared_ptr<pick_draw> pk,
                               std::shared_ptr<slice_types> c,
                               std::shared_ptr<maps> mym, int id) {
  set_basics(p, my_w, my_p, my_d, pk, c, mym);
  idat = id;
#ifdef CHEVRON
  ...
  connect(clip, SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
}
QWidget *data_sub_panel::create_clip_window() {
  ...
  contrast = new basicSliderBox(
      "Contrast", "Adjust the contrast of the dataset", 0, 0, 100);
  ...
  connect(begin->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_beg(const QString &)));
  connect(end->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_end(const QString &)));
  connect(contrast->my_sliders[0], SIGNAL(valueChanged(int)), this,
          SLOT(change_contrast(int)));
  connect(symetric->my_radios[0], SIGNAL(clicked()), this,
          SLOT(symetric_yes()));
  ...
  connect(frameOrient->my_radios[1], SIGNAL(clicked()), this, SLOT(edge()));
       connect(transpAxes->my_combos[0], SIGNAL(activated(const QString&)),this,
  SLOT(transpose(const QString&))); connect(angle1->my_edits[0],
  SIGNAL(textChanged(const QString&)),this, SLOT(angle_str(const QString&)));
          connect(angle2->my_sliders[0], SIGNAL(valueChanged(int)), this,
  SLOT(angle_int (int)));
...
QWidget *data_sub_panel::create_info_window() {
  ...
  std::vector<axis> axes =
      my_dat->return_dat(idat)->return_io_hyper()->getAxes();
  ...
  for (int iax = 0; iax < 8; iax++) {
    ...
    sprintf(temp2_ch, "n=%d o=%f d=%f label=%s", axes[iax].n, axes[iax].o,
            axes[iax].d, axes[iax].label.c_str());
    beta.append(tr(temp_ch));
    beta.append(tr(temp2_ch));
  ...
  // std::vector<QStringList> h = my_dat->return_dat(idat)->get_header(
  //   my_pan->get_panel(0)->return_orient());
  ...
  QString dat_type =
      QString::fromStdString(my_dat->return_dat(idat)->get_type());
...
void data_sub_panel::add_surface_info() {
  surfUpdate = new basicButtonBox(
      "Surface", "UPDATE", "Update the surface values with new picks", false);
  infoLay->addWidget(surfUpdate->group());
  connect(surfUpdate->my_radios[0], SIGNAL(clicked()), this,
          SLOT(update_surface()));
}
#ifdef CHEVRON
flatten_types::flatten_types() {
  s_from_v[3] = "Proportional";
  b_from_v[3] = 0;
  v_from_b[0] = 3;
  types.push_back("Proportional");
  s_from_v[44] = "Base Parallel";
  b_from_v[44] = 1;
  v_from_b[1] = 44;
  types.push_back("Base Parallel");
  s_from_v[85] = "Top Parallel";
  b_from_v[85] = 2;
  v_from_b[2] = 85;
  types.push_back("Top Parallel");
  s_from_v[126] = "Preserve";
  b_from_v[126] = 3;
  v_from_b[3] = 126;
  types.push_back("Preserve");
  s_from_v[167] = "Progradational";
  b_from_v[167] = 4;
  v_from_b[4] = 167;
  types.push_back("Progradational");
  s_from_v[208] = "Vflat";
  b_from_v[208] = 5;
  v_from_b[5] = 208;
  types.push_back("Vflat");
  s_from_v[250] = "Best guess";
  b_from_v[250] = 6;
  v_from_b[6] = 250;
  types.push_back("Best guess");
}
void data_sub_panel::add_flatten_info() {
  flatBox = new QGroupBox();
  flatLay = new QVBoxLayout();
  // flatCurrent=new basicLineEditBox("Current type",14,"Base Parallel","Type at
  // current location");

  flatChoose = new basicVRadioBox(
      "Type", flat->types[0], "Set the current location to " + flat->types[0]);
  for (int i = 1; i < (int)flat->types.size(); i++) {
    flatChoose->addButton(flat->types[i],
                          "Set the current location to " + flat->types[i]);
  }
  stratSave = new basicButtonBox("", "Save", "Save the strat collumn", false);

  connect(flatChoose->my_radios[0], SIGNAL(clicked()), this,
          SLOT(set_strat_prop()));
  connect(flatChoose->my_radios[1], SIGNAL(clicked()), this,
          SLOT(set_strat_base()));
  connect(flatChoose->my_radios[2], SIGNAL(clicked()), this,
          SLOT(set_strat_top()));
  connect(flatChoose->my_radios[3], SIGNAL(clicked()), this,
          SLOT(set_strat_pres()));
  connect(flatChoose->my_radios[4], SIGNAL(clicked()), this,
          SLOT(set_strat_prog()));
  connect(flatChoose->my_radios[5], SIGNAL(clicked()), this,
          SLOT(set_strat_vflat()));
  connect(flatChoose->my_radios[6], SIGNAL(clicked()), this,
          SLOT(set_strat_best()));
...
void data_sub_panel::add_seispak_info() {
  std::vector<std::string> keysS =
      my_dat->return_dat(idat)->return_header_keys();
  std::vector<QString> keys;
  for (int i = 0; i < keysS.size(); i++)
    keys.push_back(QString::fromStdString(keysS[i]));
  keys.insert(keys.begin(), "grid");
  extraInfoBox = new QGroupBox("SEISPAK");
  extraL = new QVBoxLayout();
  for (int i = 1; i < 8; i++) {
    seispak.push_back(new basicComboBox("AXIS " + QString::number(i + 1),
                                        "Set header key to display as axis",
                                        keys));
  }
  connect(seispak[0]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis2(const QString &)));
  connect(seispak[1]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis3(const QString &)));
  connect(seispak[2]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis4(const QString &)));
  connect(seispak[3]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis5(const QString &)));
  connect(seispak[4]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis6(const QString &)));
  connect(seispak[5]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis7(const QString &)));
  connect(seispak[6]->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_axis8(const QString &)));
...
void data_sub_panel::change_contrast(int contrast) {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    float a = (x[khi] - px) / h;
    float b = (px - x[klo]) / h;
    vals[i] = a * y[klo] + b * y[khi] +
              ((a * a * a - a) * second[klo] + (b * b * b - b) * second[khi]) *
                  (h * h) / 6.;
...
void data_sub_panel::symetric_yes() { clip->symmetric = true; }
void data_sub_panel::symetric_no() { clip->symmetric = false; }
