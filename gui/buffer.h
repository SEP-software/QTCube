...
class buffer{
  ...
  public:
    ...
    virtual void read_hyper(int *nw, int *fw){
      if(fw==0 || nw==0){;};
      par->error("Read hyper unimplemented");
    }
...
void set_hold(bool *h);
  virtual  unsigned char *get_char_data(orient_cube *pos, int iax1, int f1, int e1, int iax2,
      int f2, int e2){
       par->error("Get char data undefined");
       if(pos==0 && iax1==0 && f1==0 && e1==0 && iax2==0 && f2==0 && e2==0){;};
       return 0;   
   }
     virtual  unsigned char *get_char_data(orient_cube *pos, int n, long long *buf){
       par->error("Get char data undefined");
       if(n==0 && buf==0&&pos==0){;};
       return 0;   
   }
  virtual  float *get_float_data(orient_cube *pos, int iax1, int f1, int e1, int iax2,
      int f2, int e2){
       par->error("Get float data undefined");
       if(pos==0 && iax1==0 && f1==0 && e1==0 && iax2==0 && f2==0 && e2==0){;};
       return 0;
  }
  ...
  virtual void calc_histo(){
    par->error("Calc histo must be overriten");
  }
