      ...
      tbb::parallel_for(tbb::blocked_range<int>(0, nx),
                        [&](const tbb::blocked_range<int> &r) {
                          ...
                          for (int i2 = r.begin(); i2 != r.end(); ++i2)
                            ...
                            resampleBand(uc, i2 * ny, 1, ny, tmp, i2 * nyuse, 1,
                                         nyuse, jy, space);
                          delete[] space;
                        });

    } else
      memcpy(tmp, uc, nyuse * nx);
    if (jx > 1) {
      tbb::parallel_for(tbb::blocked_range<int>(0, nyuse),
                        [&](const tbb::blocked_range<int> &r) {
                          ...
                          for (int i1 = r.begin(); i1 != r.end(); ++i1)
                            resampleBand(tmp, i1, nyuse, nx, ucuse, i1, nyuse,
                                         nxuse, jx, space);
                          delete[] space;
                        });
    } else
...
void raster::resampleBand(const unsigned char *in, const int fin, const int jin,
                          const int nin, unsigned char *out, const int fout,
                          const int jout, const int nout, const int resamp,
                          int *space) const {
  int jw = 1 + 2 * resamp;
  int *sp1 = space;
  int *sp2 = &space[nin];
  ...
  sp1[0] = jw * in[fin];
  for (int i = 1; i < jw; i++)
    sp1[i] = (int)in[fin + jin * i] - (int)in[fin] + sp1[i - 1];
  for (int i = jw; i < nin; i++) {
    sp1[i] =
        (int)in[fin + jin * i] - (int)in[fin + jin * (i - jw)] + sp1[i - 1];
  }
  sp2[nin - 1] = sp1[nin - 1] * jw;
  for (int i = 1; i < jw; i++)
    sp2[nin - 1 - i] = sp2[nin - i] + sp1[nin - i - 1] - sp1[nin - 1];
  for (int i = jw; i < nin; i++) {
...
void raster::draw_slice(QPainter *painter, std::shared_ptr<dataset> dat,
