...
#include <wiggle.h>
using namespace SEP;
void wiggle::draw_slice(QPainter *painter, std::shared_ptr<dataset> dat,
  ...
  PaintWiggles(n2, n1, painter, bufA);
...
void wiggle::PaintWiggles(int n2, int n1, QPainter *painter,
                          unsigned char *buf) {
  painter->save();
  ...
  if (neg_color.contains("none") > 0)
    do_neg = false;
  if (pos_color.contains("none") > 0)
    do_pos = false;
  // int nmax=ndraw;
  int j2 = 1;
  if (ndraw > n2)
  ...
  for (int n2Counter = 0; n2Counter < n2; n2Counter += j2, itr++) {
    ...
    // Divide the range by the number of possible unsigned chars, yielding the
    // change in x per char
    ...
    for (int n1Counter = 0; n1Counter < n1; n1Counter++) {
      ...
      painter->setPen(pen);
      if (do_pos) {
        // Regions of data to the right of the value 128 (ie positive in the
        // unsigned char scale) are shaded in.
        if (x > xzero || prevX > xzero) {
          // If the yRange us one or less, the regions are shaded in by
          // horizontal lines.
          ...
          // Otherwise they are shaded in as polygons made up of combinations of
      ...
      if (do_neg) {
        ...
        // Regions of data to the left of the value 128 (ie negative in the
        // unsigned char scale) are shaded in.
        if (x < xzero || prevX < xzero) {
          // If the yRange us one or less, the regions are shaded in by
          // horizontal lines.
