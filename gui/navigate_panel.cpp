...
using namespace SEP;
navigate_panel::navigate_panel(std::shared_ptr<position> p,
...
QWidget *navigate_panel::build_row_1() {
  ...
  direction =
      new basicRadioBox("Direction", "x", "Move in the positive x direction");
  ...
  increment = new basicLineEditBox("Increment", 3, "1",
                                   "Number of samples to move with each step");

  lay1->addWidget(direction->group());
  lay1->addWidget(increment->group());
  lay1->addStretch(1);
  mine->setLayout(lay1);
  connect(direction->my_radios[0], SIGNAL(clicked()), this,
          SLOT(x_pos_clicked()));
  connect(direction->my_radios[1], SIGNAL(clicked()), this,
          SLOT(x_neg_clicked()));
  connect(direction->my_radios[2], SIGNAL(clicked()), this,
          SLOT(y_pos_clicked()));
  connect(direction->my_radios[3], SIGNAL(clicked()), this,
          SLOT(y_neg_clicked()));
  connect(direction->my_radios[4], SIGNAL(clicked()), this,
          SLOT(z_pos_clicked()));
  connect(direction->my_radios[5], SIGNAL(clicked()), this,
          SLOT(z_neg_clicked()));
  connect(increment->my_edits[0], SIGNAL(textChanged(const QString &)), this,
          SLOT(inc_changed(const QString &)));
...
QWidget *navigate_panel::build_row_2() {
  ...
  step = new basicButtonBox(
      "Step view", "Step", "Take a step in the current movie direction", false);
  ...
  connect(speed->my_sliders[0], SIGNAL(valueChanged(int)), this,
          SLOT(speed_changed(int)));
...
QWidget *navigate_panel::build_row_3() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    pos.push_back(new basicLineEditBox(tr("Axis ") + QString::number(i + 1), 10,
                                       QString::number(a.o + a.d * a.n / 2.),
                                       "Current position"));
  }
  update =
      new basicButtonBox("Position", "Update",
                         "Update the current position being displayed", false);
  ...
  connect(update->my_radios[0], SIGNAL(clicked()), this,
          SLOT(update_clicked()));
...
void navigate_panel::update_clicked() {
  ...
  if (v) {
    if (validax[1]) {
      a = pos[1]->my_edits[0]->displayText();
      poz[1] = a.toFloat(&v);
    } else
      poz[1] = 0.;
    if (v) {
      if (validax[2]) {
        a = pos[2]->my_edits[0]->displayText();
        poz[2] = a.toFloat(&v);
      } else
        poz[2] = 0.;
      if (v) {
        if (validax[3]) {
          a = pos[3]->my_edits[0]->displayText();
          poz[3] = a.toFloat(&v);
        } else
          poz[3] = 0.;
        if (v) {
          if (validax[4]) {
            a = pos[4]->my_edits[0]->displayText();
            poz[4] = a.toFloat(&v);
          } else
            poz[4] = 0.;
          if (v) {
            if (validax[5]) {
              a = pos[5]->my_edits[0]->displayText();
              poz[5] = a.toFloat(&v);
            } else
              poz[5] = 0.;
            if (v) {
              if (validax[6]) {
                a = pos[6]->my_edits[0]->displayText();
                poz[6] = a.toFloat(&v);
              } else
                poz[6] = 0.;
              if (v) {
                if (validax[7]) {
                  a = pos[7]->my_edits[0]->displayText();
                  poz[7] = a.toFloat(&v);
                } else
                  poz[7] = 0.;
                //  window->return_grid_loc(pos,iloc);
                a = QString::fromStdString(
                    util::string_from_float_array(8, poz));
                coms.push_back(a);
                emit actionDetected(coms);
              }
            }
