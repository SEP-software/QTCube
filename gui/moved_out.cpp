#include "moved_out.h"
using namespace SEP;
void moved_out::set_moveout_basics(std::shared_ptr<hypercube> g,
                                   std::shared_ptr<Qdataset> dat, const int it,
...
void moved_out::read_block_float(std::vector<int> &nw, std::vector<int> &fw,
                                 float *buf) {
...
void moved_out::read_block_byte(std::vector<int> &nw, std::vector<int> &fw,
                                unsigned char *buf) {
  if (nw[i_t] != grid->getAxis(i_t + 1).n ||
      nw[i_off] != grid->getAxis(i_off + 1).n)
    par->error("Internal error in measure");
