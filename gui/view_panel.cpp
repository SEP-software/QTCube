...
using namespace SEP;
view_panel::view_panel(std::shared_ptr<position> p,
...
QWidget *view_panel::build_row_1() {
  ...
  for (std::map<QString, bool>::iterator i = typ->begin(); i != typ->end(); i++)
    types.push_back(i->first);

  perspective = new basicComboBox("Perspective", "Perspective to use", types);
  cycleData = new basicButtonBox("Cycle data", ":/images/back.png",
                                 "Switch back one dataset", true);
  ...
  // std::vector<std::string> dislistS = my_c->return_cvec();
  std::vector<QString> dislist = my_c->return_cvec();
  ;
  // for (auto i = 0; i < dislistS.size(); i++) dislist.push_back(dislistS[i]);
  mainType = new basicComboBox("Display", "Type of display", my_slices->names);

  connect(perspective->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(perspective_change(const QString &)));
  connect(cycleData->my_radios[1], SIGNAL(clicked()), this,
          SLOT(roundRobinForward()));
  connect(cycleData->my_radios[0], SIGNAL(clicked()), this,
          SLOT(roundRobinBack()));
  connect(mainType->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(display_change(const QString &)));
  connect(data->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(data_change(const QString &)));
...
QWidget *view_panel::build_row_2() {
  ...
  opacity = new basicSliderBox("Opacity", "Relative of opacity of overlay", 0,
                               128, 256);
  cycleOver = new basicButtonBox("Cycle overlay data", ":/images/back.png",
                                 "Switch back one dataset", true);
  cycleOver->addButton(":/images/forward.png", "Switch back one dataset", true);
  overType = new basicComboBox("Overlay Display", "Type of display for overlay",
                               my_slices->names);
  ...
  connect(cycleOver->my_radios[1], SIGNAL(clicked()), this,
          SLOT(oRoundRobinForward()));
  connect(cycleOver->my_radios[0], SIGNAL(clicked()), this,
          SLOT(oRoundRobinBack()));
  connect(overType->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(odisplay_change(const QString &)));
  connect(over->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(odata_change(const QString &)));
  connect(opacity->my_sliders[0], SIGNAL(valueChanged(int)), this,
          SLOT(update_opactiy(int)));
...
QWidget *view_panel::map1_tab() {
  ...
  overSamp = new basicLineEditBox(
      "Oversample", 7, "1.",
      "Oversample factor of map compared to data being displayed");
  loadMap1 =
      new basicButtonBox("Load", ":/images/open.png", "Load new map", true);

  usingMap1->set_current(0, 0);

  connect(usingMap1->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_map1(const QString &)));
...
QWidget *view_panel::wiggle_tab() {
  ...
  posFill = new basicComboBox("Positive fill", "Fill color for positive values",
                              color);
  negFill = new basicComboBox("Negative fill", "Fill color for negative values",
                              color);
  lineColor->set_current(0, 8);
  posFill->set_current(0, 9);
  negFill->set_current(0, 0);
  connect(lineColor->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(wiggle_color_change(const QString &)));
  connect(posFill->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(wiggle_pos_change(const QString &)));
  connect(negFill->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(wiggle_neg_change(const QString &)));
...
QWidget *view_panel::grid_tab() {
  ...
  sampleGrid = new basicLineEditBox("Sampling", 7, "1",
                                    "Increment in third axis to display");
  connect(sampleGrid->my_edits[0], SIGNAL(textChanged(const QString &)), this,
          SLOT(change_jplane(const QString &)));
  connect(grid1->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_grid1(const QString &)));
  connect(grid2->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(change_grid2(const QString &)));
...
QWidget *view_panel::contour_tab() {
  ...
  sampleContour =
      new basicLineEditBox("sampling", 7, "", "Sampling of contours");

  connect(contourColor->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(contour_color_change(const QString &)));
  connect(numLines->my_combos[0], SIGNAL(activated(const QString &)), this,
          SLOT(num_lines_change(const QString &)));
  connect(firstContour->my_edits[0], SIGNAL(textChanged(const QString &)), this,
          SLOT(first_contour_change(const QString &)));
  connect(sampleContour->my_edits[0], SIGNAL(textChanged(const QString &)),
          this, SLOT(sample_contour_change(const QString &)));
