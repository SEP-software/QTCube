...
surface_data::surface_data(hypercube *g, QString nm, QString typ, dataset *dt,param_func *p,
  ...
  buf=new float[nbuf];
fprintf(stderr,"CHECK NBUF %d \n",nbuf);
my_orient=-1;
}
void surface_data::create_buffer(orient_cube *pos){
...
void surface_data::update_surface(){
 ...
 picks_vec *mn=pk->return_all_picks(my_pos,pick_col);

 for(long long i=0; i < nbuf; i++) buf[i]=FUNKY_VAL;

 orient_cube ori=orient_cube(my_pos);
...
unsigned char *surface_data::get_char_data(orient_cube *pos,int iax1,int iax2, int n, long long *index){
...
float *surface_data::get_float_data(orient_cube *pos,int iax1,int iax2, int n, long long *index){
