...
#include "math.h"
using namespace SEP;
void dataset::set_basics(std::string titl, QString nm,
...
int dataset::check_load_buffer(std::shared_ptr<orient_cube> pos, int iax1,
                               int iax2) {
...
void dataset::delete_dataset(std::shared_ptr<orient_cube> pos, int iax1,
                             int iax2) {
...
QString dataset::return_clip() {
  ...
  for (int i = 1; i < (int)clip.size(); i++) {
    buf = buf + ":" + QString::number(clip[i].pct) + ":" +
          QString::number(clip[i].clip);
  }
  return buf;
}
...
void dataset::build_conv() {
  ...
  com.push_back(QString::number(inum));
  com.push_back("clip");
  com.push_back("set_pts");
  com.push_back(return_clip());
