...
void datasets::clean_dats() { dats.clear(); }
std::shared_ptr<Qdataset> datasets::return_dat(int idat) {
...
void datasets::update_data(std::vector<QString> command) {
  ...
  if (command[2].contains("add") > 0) {
    ...
    dats[idat]->set_display_axis(iax - 1, command[4].toStdString());
  } else if (command[2].contains("set") > 0) {
    float *pct = util::float_array_from_string(command[3].toStdString());
    QString tmp = QString::fromStdString(dats[idat]->set_pts(pct));
    dats[idat]->add_pt(command[3].section(":", 0, 0).toFloat(),
                       command[3].section(":", 1, 1).toFloat());
    ...
    if (command[3].contains("update") > 0) {
      ...
      std::shared_ptr<surface_data> x =
          std::static_pointer_cast<surface_data>(r->_data);
      x->update_surface();
...
std::shared_ptr<Qdataset> datasets::return_dat(QString nm) {
  for (int i = 0; i < (int)dats.size(); i++) {
    if (dats[i]->get_name() == nm.toStdString()) return dats[i];
  }
  // par->error("invalid dataset");
  return NULL;
}
...
void datasets::add_dat(std::shared_ptr<Qdataset> d) {
  connect(d.get(), SIGNAL(actionDetected(std::vector<QString>)), this,
...
int datasets::dataNumFromName(QString name) {
  for (int i = 0; i < return_ndat(); i++) {
    if (name == QString::fromStdString(dats[i]->get_name())) return i;
  }
  return 0;
}
