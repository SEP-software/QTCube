...
*/
void slice::set_draw_params(std::shared_ptr<QFontMetrics> f_m,
                            std::shared_ptr<orient_cube> pos, int i1, int i2,
  ...
  reverse1 =
      ((pos->get_reverse(iax1) || rev1) && (rev1 != pos->get_reverse(iax1)));
  reverse2 =
  ...
  if ((iax1 == order[0] && iax2 == order[1]) ||
      (iax2 == order[0] && iax1 == order[1]))
    i3_axis = order[2];
  if ((iax1 == order[0] && iax2 == order[2]) ||
      (iax2 == order[0] && iax1 == order[2]))
    i3_axis = order[1];
  if ((iax1 == order[2] && iax2 == order[1]) ||
...
void slice::form_pixel_slice_map() {
  ...
  n1 = e_1 - b_1;
  my_pos->getAxis_range(iax2, &b_2, &e_2);
  n2 = e_2 - b_2;
...
void slice::draw_axes(QPainter *painter, std::shared_ptr<orient_cube> pos,
  ...
  if (display1 < 0)
    tic_loc1 =
        this->optimal_with_font(ye - yb, mn - ax1.d / 2., mx + ax1.d / 2.);
  else
  ...
  if (reverse1 && display1 >= 0) {
    ...
    for (int i = 0; i < (int)junk.size(); i++)
      tic_loc1[tic_loc1.size() - 1 - i] = junk[i];
    junk.clear();
    for (int i = 0; i < (int)tic_val1.size(); i++)
      junk.push_back(tic_val1[i]);
    for (int i = 0; i < (int)junk.size(); i++)
      tic_val1[tic_val1.size() - 1 - i] = junk[i];
  }
  ...
  aa = ax1.label.c_str();
  if (display1 >= 0)
    aa = QString::fromStdString(my_dat->return_axis_name(iax1));
  pw = fm->width(aa);
  ph = fm->height();
  ...
  if (display2 < 0)
    tic_loc2 =
        this->optimal_with_font(xe - xb, mn - ax2.d / 2., mx + ax2.d / 2.);
  else
    tic_loc2 = this->optimal_with_font(xe - xb, mn, mx, ax_val2, &tic_val2);
  ...
  if (reverse2 && display2 >= 0) {
    ...
    for (int i = 0; i < (int)junk.size(); i++)
      tic_loc2[tic_loc2.size() - 1 - i] = junk[i];
    junk.clear();
    for (int i = 0; i < (int)tic_val2.size(); i++)
      junk.push_back(tic_val2[i]);
    for (int i = 0; i < (int)junk.size(); i++)
      tic_val2[tic_val2.size() - 1 - i] = junk[i];
  }
  ...
  aa = ax2.label.c_str();
  if (display2 >= 0)
    aa = QString::fromStdString(my_dat->return_axis_name(iax2));
...
bool slice::in_slice(int ix, int iy) {
  ...
  if (i_x >= xb && i_x <= xe && i_y >= yb && i_y <= ye)
    ret = true;
  return ret;
}
void slice::update_pos(std::vector<QString> *com, int ix, int iy,
  ...
  if (this->in_slice(ix, iy) == true) {
    ...
    // This is tricky. This is my current position in the unrotated coordinate
    // system.
...
void slice::update_range(std::vector<QString> *com, int ix_old, int iy_old,
  ...
  if (this->in_slice(ix, iy) == true) {
    ...
    int loc[8];
    pos->get_locs(loc); // RGC-This maybe should be differently when in a
                        // rotated/stretched coordinate
...
bool slice::get_image_pos_sample(float ix, float iy, int *id1, int *id2) {
  ...
  // IO_loc assumes an integer so we are going to round to the nearest sample
  // and then redo the fraction
  ...
  // fprintf(stderr,"l2ock int %d %d %f %f %d %d %d
  // %d\n",loc[iax1],loc[iax2],fx,fy,
  // b1_grid,e1_grid,b2_grid,e2_grid);

  float pctx =
      ((float)(loc[iax2] + fx - b2_grid) / ((float)(e2_grid - b2_grid)));
  float pcty =
      ((float)(loc[iax1] + fy - b1_grid) / ((float)(e1_grid - b1_grid)));
...
std::vector<float> slice::optimal_with_font(int axis_size, int mn, int mx,
  ...
  int nguess =
      axis_size / (nc + 5); // Approximate number of samples we could display
...
int slice::optimal_scale_linear(int n, float min, float max, float *onum,
  ...
  if (min <= max) {
    ...
    if ((hi + 1) * nice <= (max - delta * range) + FLT_EPSILON)
      hi++;
    *onum = lo * nice;
    *dnum = nice;
  } else {
    ...
    if ((hi + 1) * nice <= min + FLT_EPSILON + delta * range)
      hi++;
