...
using namespace SEP;
basicComboBox::basicComboBox(QString name, QString tool,
                             std::vector<QString> options) {
  my_box = new QGroupBox(name);
  my_box->setStyleSheet("padding: 10px");
...
void basicComboBox::addWidget(QString name, QString tool,
                              std::vector<QString> options) {
  ...
  my_combos[i]->setStyleSheet("padding: 6px");
...
basicLineEditBox::basicLineEditBox(QString box, int length, QString def,
                                   QString tool) {
...
void basicRadioBox::set_checked(int i) { my_radios[i]->setChecked(true); }
...
void basicVRadioBox::set_checked(int i) { my_radios[i]->setChecked(true); }
...
basicButtonBox::basicButtonBox(QString box, QString name, QString tool,
                               bool icon) {
  my_box = new QGroupBox(box);
  my_lay = new QHBoxLayout();
...
basicSliderBox::basicSliderBox(QString box, QString tool, int mn, int def,
                               int mx) {
  my_box = new QGroupBox(box);
  my_lay = new QHBoxLayout();
