...
void surface_data::update_surface() {
  ...
  for (int i = 1; i < 8; i++) {
    axis a = h->getAxis(i);
    if (i - 1 != isingle)
      nblock[i] = nblock[i - 1] * (long long)a.n;
    else {
      nblock[i] = nblock[i - 1];
    }
  }
...
unsigned char *surface_data::get_char_data(std::shared_ptr<orient_cube> pos,
...
float *surface_data::get_float_data(std::shared_ptr<orient_cube> pos, int iax1,
  ...
  for (int i = 1; i < 8; i++) {
    ...
    mult[i] = 1;
    if (i - 1 == isingle) {
      nblocko[i] = nblocko[i - 1];
    } else
      nblocko[i] = nblocko[i - 1] * a.n;
...
unsigned char *surface_data::float_to_byte(int n, float *tb) {
  ...
  for (int i = 0; i < n; i++) {
    if (tb[i] == FUNKY_VAL)
      ...
      buf[i] =
          (int)std::max(1, std::min(255, (int)(((tb[i] - minv) / d) + 1.)));
  }
  return buf;
}
float surface_data::get_value(std::shared_ptr<orient_cube> pos) {
  ...
  for (int i = 0; i < (int)slices.size(); i++) {
    if (slices[i]->have_slice(display, pick_col, isingle, pos, slices[i]->iax1,
                              slices[i]->iax2)) {
...
float surface_data::get_minval() { return minv; }
float surface_data::get_maxval() { return maxv; }
float *surface_data::check_load_slice(std::shared_ptr<orient_cube> pos,
                                      int iax1, int f1, int e1, int iax2,
                                      int f2, int e2) {
  for (int i = 0; i < (int)slices.size(); i++) {
  ...
  std::shared_ptr<surface_slice> x(
      new surface_slice(pos, iax1, iax2, isingle, display, pick_col, dat, pk));
  slices.push_back(x);
...
void surface_slice::form_depth_slice(std::shared_ptr<orient_cube> ori,
                                     std::shared_ptr<picks_vec> all, float o,
                                     float d) {
  if (o == 0 && d == 0) {
    ;
  }
...
float *surface_slice::return_slice(std::shared_ptr<orient_cube> pos, int f1,
                                   int e1, int f2, int e2) {
  int n1s = abs(e1 - f1);
  int n2s = abs(e2 - f2);
  int j1 = 1, j2 = 1;
  if (f1 > e1) j1 = -1;
  ...
  int i = 0;
  // ThERE IS A BUG IN THE CURRENT FORM THIS EXPECTS FULL SLICE WHILE WE ARE
  // CREATING What is currently visible.  Choose to either never allow rotation
  // or track rotation map
  if (pos->get_rotate()) {
