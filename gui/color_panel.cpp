...
using namespace SEP;
color_panel::color_panel(std::shared_ptr<position> p,
  ...
  loadsave = new basicButtonBox("Read/Write", ":./images/open.png",
                                "Load a color table", true);
  loadsave->addButton(":./images/save.png", "Save a color table", true);

  std::vector<QString> ctypes;
  ctypes.push_back("grey");
  ctypes.push_back("flag");
  ctypes.push_back("cbl");
  ctypes.push_back("cgsi");
  ctypes.push_back("rainbow");
  ctypes.push_back("ava");
  ctypes.push_back("patriotic");
  ctypes.push_back("jet");
  ...
  std::shared_ptr<user_color> us =
      std::static_pointer_cast<user_color>(my_slices->slices["user"]);
  ...
  setLayout(mainLay);
  connect(cbar.get(), SIGNAL(actionDetected(std::vector<QString>)), this,
...
void color_bar2::load_cbar(QString text) {
  ...
  float val;
  char line[2048];
  if ((fd = fopen(text.toLatin1(), "r")) == NULL)
...
void color_bar2::save_cbar(QString text) {
  FILE *fd;
  fd = fopen(text.toLatin1(), "w");
  for (int i = 0; i < 33; i++) {
    if (maps["red"][i] > -.0001)
      fprintf(fd, "%s %d %f \n", "red", i, maps["red"][i]);
    if (maps["green"][i] > -.0001)
...
void color_bar2::extract_draw(QString col, QPainter *painter) {
  ...
  for (int i = 0; i < (int)pts.size(); i++) {
    ...
    pt.putPoints(0, 5, ix - 2, iy - 2, ix + 2, iy - 2, ix + 2, iy + 2, ix - 2,
                 iy + 2, ix - 2, iy - 2);
