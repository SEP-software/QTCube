...
class pick_param {
 public:
  ...
  void draw(QPainter *painter, int iax1, int iax2,
            std::shared_ptr<SEP::pairs_new> myp, float rat);
  void draw_points(QPainter *painter, std::shared_ptr<SEP::pairs_new> myp,
                   bool txt);
  void draw_line(QPainter *painter, std::shared_ptr<SEP::pairs_new> myp,
                 float rat);
...
class pick_draw {
 public:
  ...
  int get_display();
  void change_active_col(QString name) { active_col = name; }
  ...
  void update_slice_maps(int *iloc, int *inew, int iax1, int iax2, int iax3,
                         std::shared_ptr<SEP::slice> slc);
  ...
  void add_rotated_pt(int orient_num, int *iloc, int type, QString col = "None",
                      int extra = -99, QString txt = "None");
  void del_nearest(std::shared_ptr<SEP::orient_cube> pos, int *floc, int iax1,
                   int iax2);
  void add_pt(int *iloc, int type, QString col = "None", int extra = -99,
              QString txt = "None");
  void del_pt(std::shared_ptr<SEP::orient_cube> pos, int *floc, int iax1,
              int iax2);
  void delete_picks(std::shared_ptr<SEP::orient_cube> pos, int ival,
                    QString col = "None");
  ...
  std::shared_ptr<SEP::picks_vec> return_all_picks(
      std::shared_ptr<SEP::orient_cube> pos, QString col);
  std::shared_ptr<SEP::picks_vec> return_all_plane(int *iloc, int iax1,
                                                   int iax2, QString col);
  std::shared_ptr<SEP::picks_vec> return_all_plane_single(int *iloc, int iax1,
                                                          int iax2,
                                                          QString col);
  void draw(QPainter *painter, std::shared_ptr<SEP::slice> slc);
  void clear_picks(std::shared_ptr<SEP::orient_cube> pos, int *iloc,
                   QString interval);
  void del_pick_orients(std::shared_ptr<SEP::pick_new> pk);

  std::shared_ptr<pairs_new> get_pts_sort(
      std::shared_ptr<SEP::orient_cube> orient_num, int *iloc, int i1,
      int isort, QString col);
  std::shared_ptr<picks_vec> create_load_plane(
      std::shared_ptr<SEP::orient_cube> pos, int iax1, int iax2, int *iloc);
  std::shared_ptr<picks_vec> return_iloc_based(int *iloc, QString col);
  std::shared_ptr<pairs_new> get_pts_sort_le(
      std::shared_ptr<SEP::orient_cube> pos, int iax1, int iax2, int ival,
      int isort, QString col);
  void add_multi_picks(std::vector<long long> locs);
  void delete_multi_picks(std::vector<long long> locs);
  void set_pick_text(QString t) {
    active_txt = t;
    fprintf(stderr, "Setting text %s \n", active_txt.toLatin1().constData());
  }
  void ignore_axis(int i) {
    if (buck)
      fprintf(stderr, "see axis \n");
    else
      fprintf(stderr, "Can not see bucket \n");
    buck->ignore_axis(i);
  }
...
picking();
  ...
  pairs_new get_pts_sort(int *iloc,int iax1, int iax2, int isort, int
ival,QString col="None"); pairs_new
get_pts_sort(std::shared_ptr<orient_cube>pos,std::shared_ptr<slice>slc,int
ival,int isort, QString col="None"); pairs_new get_pts_sort_le(int *iloc,int
iax1, int iax2, int isort, int ival,QString col="None"); pairs_new
get_pts_sort_le(std::shared_ptr<orient_cube>pos,int *iloc,int iax1, int iax2,
int isort, int ival,QString col="None"); pairs_new
get_pts_sort_le(std::shared_ptr<orient_cube>pos,std::shared_ptr<slice>slc,int
ival,int isort, QString col="None"); std::vector<pick*>
all_picks(std::shared_ptr<orient_cube>pos,int *iloc, int iax1, int iax2, QString
col); std::vector<pick*> all_picks(int *iloc, int iax1, int iax2, int
ising,QString col);
