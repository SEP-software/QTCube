...
#include "wiggle.h"
using namespace SEP;
panels::panels(std::shared_ptr<genericIO> io, std::shared_ptr<pick_draw> p,
               std::shared_ptr<maps> mym, std::shared_ptr<orients> oo) {
  pan_num = 0;
  locked = false;
  pk = p;
  my_maps = mym;
...
void panels::perform_orient(std::vector<QString> command) {
  ...
  if (command[2].contains("flip1") > 0) {
    ...
    if (command[3].contains("12") > 0) {
      new_order[1] = order[0];
      new_order[0] = order[1];
    } else if (command[3].contains("13") > 0) {
      new_order[2] = order[0];
      new_order[0] = order[2];
    } else if (command[3].contains("14") > 0) {
      new_order[3] = order[0];
      new_order[0] = order[3];
    } else if (command[3].contains("15") > 0) {
      new_order[4] = order[0];
      new_order[0] = order[4];
    } else if (command[3].contains("16") > 0) {
      new_order[5] = order[0];
      new_order[0] = order[5];
    } else if (command[3].contains("17") > 0) {
      new_order[6] = order[0];
      new_order[0] = order[6];
    } else if (command[3].contains("18") > 0) {
      new_order[7] = order[0];
      new_order[0] = order[7];
    } else if (command[3].contains("23") > 0) {
      new_order[2] = order[1];
      new_order[1] = order[2];
    } else if (command[3].contains("24") > 0) {
      new_order[3] = order[1];
      new_order[1] = order[3];
    } else if (command[3].contains("25") > 0) {
      new_order[4] = order[1];
      new_order[1] = order[4];
    } else if (command[3].contains("26") > 0) {
      new_order[5] = order[1];
      new_order[1] = order[5];
    } else if (command[3].contains("27") > 0) {
      new_order[6] = order[1];
      new_order[1] = order[6];
    } else if (command[3].contains("28") > 0) {
      new_order[7] = order[1];
      new_order[1] = order[7];
    } else if (command[3].contains("34") > 0) {
      new_order[3] = order[2];
      new_order[2] = order[3];
    } else if (command[3].contains("35") > 0) {
      new_order[4] = order[2];
      new_order[2] = order[4];
    } else if (command[3].contains("36") > 0) {
      new_order[5] = order[2];
      new_order[2] = order[5];
    } else if (command[3].contains("37") > 0) {
      new_order[6] = order[2];
      new_order[2] = order[6];
    } else if (command[3].contains("38") > 0) {
      new_order[7] = order[2];
      new_order[2] = order[7];
    } else if (command[3].contains("45") > 0) {
      new_order[4] = order[3];
      new_order[3] = order[4];
    } else if (command[3].contains("46") > 0) {
      new_order[5] = order[3];
      new_order[3] = order[5];
    } else if (command[3].contains("47") > 0) {
      new_order[6] = order[3];
      new_order[3] = order[6];
    } else if (command[3].contains("48") > 0) {
      new_order[7] = order[3];
      new_order[3] = order[7];
    } else if (command[3].contains("56") > 0) {
      new_order[5] = order[4];
      new_order[4] = order[5];
    } else if (command[3].contains("57") > 0) {
      new_order[6] = order[4];
      new_order[4] = order[6];
    } else if (command[3].contains("58") > 0) {
      new_order[7] = order[4];
      new_order[4] = order[7];
    } else if (command[3].contains("67") > 0) {
      new_order[6] = order[5];
      new_order[5] = order[6];
    } else if (command[3].contains("68") > 0) {
      new_order[7] = order[5];
      new_order[5] = order[7];
    } else if (command[3].contains("78") > 0) {
      new_order[7] = order[6];
      new_order[6] = order[7];
    }
    for (int i = 0; i < 8; i++) new_order[i]++;
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->return_orient()->set_orders(new_order);
    update_it(active);

  } else if (command[2].contains("angle") > 0) {
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->return_orient()->set_rot_angle(command[3].toFloat());
    update_it(active);
    ...
    for (int i1 = 1; i1 < 9; i1++) {
      for (int i2 = i1 + 1; i2 < 9; i2++) {
        ...
        if (command[3].contains(test)) {
          for (int i = ibeg; i <= iend; i++)
            my_pan[i]->return_orient()->set_rot_axes(i1 - 1, i2 - 1);
        }
      }
    }
    update_it(active);
    ...
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->return_orient()->set_rot_angle(0);
    update_it(active);
    ...
    pos->get_locs(iloc);
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->return_orient()->set_rot_pt(iloc);
    update_it(active);
    ...
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->move_corner_beg(command[3].toInt(), command[4].toInt());
    update_it(active);
  } else if (command[2].contains("vsize_end")) {
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->move_corner_end(command[3].toInt(), command[4].toInt());
    update_it(active);
  } else if (command[2].contains("vsize")) {
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->moving_corner(command[3].toInt(), command[4].toInt());
    update_it(active);
  }
}
...
void panels::run_movie() { movie_timer->start(movie_delay); }
void panels::update_movie() {
  my_pan[0]->return_orient()->update_movie_pos(
      my_pan[0]->return_dat()->return_io_hyper());
  sync_panel(my_pan[0]);
  update_all();
  emit windows_update();
}
void panels::stop_movie() { movie_timer->stop(); }
void panels::perform_navigate(std::vector<QString> command) {
  ...
  if (command[2].contains("direction") > 0) {
    ...
    if (command[3].contains("x") > 0) {
      nax = 1;
      ndir = delta;
    } else if (command[3].contains("X") > 0) {
      nax = 1;
      ndir = -1;
    } else if (command[3].contains("y") > 0) {
      nax = 2;
      ndir = 1;
    } else if (command[3].contains("Y") > 0) {
      nax = 2;
      ndir = -1;
    } else if (command[3].contains("z") > 0) {
      nax = 0;
      ndir = 1;
    } else if (command[3].contains("Z") > 0) {
      nax = 0;
    ...
    for (int i = 0; i < (int)my_pan.size(); i++)
      my_pan[i]->return_orient()->set_movie_dir(move_axis, ndir);
    pos->update_movie_pos(active->return_dat()->return_io_hyper(), delta);
    sync_panel(active);
    update_all();
    ...
    std::vector<int> myp =
        util::int_vector_from_string(command[3].toStdString());
...
void panels::perform_view(std::vector<QString> command) {
  ...
  if (command[2].contains("map1")) {
    if (command[3].contains("load")) {
      std::shared_ptr<map_1> x(new map_1(_io, command[5],
                                         active->return_dat()->return_grid(),
                                         command[4].toFloat()));
      my_maps->add_map(x);
      active->change_map(my_maps->size() - 1);
      update_it(command[0].toInt());
    } else if (command[3].contains("change")) {
      if (command[4].contains("None"))
        active->set_no_map();
      else {
    ...
    for (int i = ibeg; i <= iend; i++) {
      if (command[3].toInt() == -1)
        my_pan[i]->set_overlay(false);
      else {
        my_pan[i]->set_overlay(true);
    ...
    std::shared_ptr<slice> x =
        active->get_colortables()->return_color_table(std::string("wiggle"));
    wigit = std::static_pointer_cast<wiggle>(x);
    wigit->set_wiggle_color(command[3]);
    update_it(command[0].toInt());

  } else if (command[2].contains("wiggle_pos") > 0) {
    wigit = std::static_pointer_cast<wiggle>(
        active->get_colortables()->return_color_table(std::string("wiggle")));
    ...
    cont = std::static_pointer_cast<contour>(
        active->get_colortables()->return_color_table(std::string("contour")));
    ...
    for (int i = ibeg; i <= iend; i++)
      my_pan[i]->set_jplane(command[3].toInt());
    update_it(command[0].toInt());
...
void panels::perform_auto(std::vector<QString> command) {
  ...
  if (command[2].contains("method") > 0) {
    ...
    if (command[3].contains("amplitude"))
      autop->set_amplitude();
    else
      autop->set_correlate();
...
void panels::perform_pick(std::vector<QString> command) {
  ...
  if (command[2].contains("multi_add") > 0) {
    ...
    for (int i = 0; i < nlen; i++)
      locs.push_back(command[4].section(":", i, i).toLongLong());
    ...
    QString news;
    news = command[3];
    int *iloc = util::int_array_from_string(command[3].toStdString());
    ...
    news = QString::fromStdString(util::string_from_int_array(11, iloc));
    iloc[9] = -iloc[9];
    update_all();
    ...
    int new_display;
    if (command[3].contains("line"))
      new_display = DISPLAY_LINE;
    else if (command[3].contains("points"))
      new_display = DISPLAY_POINTS;
    else if (command[3].contains("text"))
...
void panels::update_what(int iv, QString what) {
  if (what.contains("all"))
    update_all();
  else if (what.contains("only"))
    update_only(iv);
  else if (what.contains("limited"))
    update_it(iv);
}
...
void panels::connect_it(int i) {
  connect(my_pan[i].get(), SIGNAL(actionDetected(std::vector<QString>)), this,
          SLOT(actionRespond(std::vector<QString>)));
}
void panels::disconnect_it(int i) {
  disconnect(my_pan[i].get(), SIGNAL(actionDetected(std::vector<QString>)),
