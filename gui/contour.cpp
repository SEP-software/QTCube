...
void contour::draw_slice(QPainter *painter, std::shared_ptr<dataset> dat,
  ...
  int bb1, bb2, ee1, ee2;
  if ((rev_1 && !pos->get_reverse(iax1)) ||
      (!rev_1 && pos->get_reverse(iax1))) {
    ee1 = f1;
    bb1 = f1 + n1;
  } else {
    bb1 = f1;
  ...
  if ((rev_2 && !pos->get_reverse(iax2)) ||
      (!rev_2 && pos->get_reverse(iax2))) {
    ee2 = f2;
    bb2 = f2 + n2;
  ...
  painter->setPen(*pen);
  if (axis_top || axis_bot || axis_left || axis_right)
    this->draw_axes(painter, pos, draw_grid);
  this->draw_pos(painter, pos);
}

int contour::cconnect(float *z, int n1, int n2, float c, int *ix, int *iy,
                      bool *south_mark, bool *west_mark, QPolygon *pts) {
  int jx, jy;
  float x, y;
  int i_x, i_y;
  jx = (*ix);
  ...
  if (south_mark[(jy + 1) * n1 + jx]) /* if exiting north */
  {
    ...
    if (++(*iy) >= n2 - 1) return (0);
  } else if (west_mark[(jy)*n1 + jx + 1]) /* if exiting east */
  {
    jx++;
    x = jx;
    ...
    if (++(*ix) >= n1 - 1) return (0);
  } else if (south_mark[(jy)*n1 + jx]) /* if exiting south */
  {
    x = jx + delta(c, z[jy * n1 + jx], z[jy * n1 + jx + 1]);
    y = jy;
    get_shifted_image_pos_fract(x, y, &i_x, &i_y);
    ...
    south_mark[jy * n1 + jx] = false;
    if (--(*iy) < 0) return (0);
  } else if (west_mark[(jy)*n1 + jx]) /* if exiting west */
  {
    x = jx;
    y = jy + delta(c, z[jy * n1 + jx], z[(jy + 1) * n1 + jx]);
    get_shifted_image_pos_fract(x, y, &i_x, &i_y);
...
int contour::contour_val(QPainter *painter, float *z, bool *west_mark,
                         bool *south_mark, int n1, int n2, float c) {
  int ix, iy, non;
  int jx, jy;
  ...
  for (iy = 0; iy < n2 - 1 && non > 0; iy++) {
    for (ix = 0; ix < n1 - 1 && non > 0; ix++) {
      if (south_mark[iy * n1 + ix]) /* check south edge of cell */
      {
        ...
        draw_contour(c, painter, &polygon);
        while (
            cconnect(z, n1, n2, c, &jx, &jy, south_mark, west_mark, &polygon))
          non--;
        draw_contour(c, painter, &polygon);
      }
    }
  ...
  while (!found && i < poly->size() - 1) {
    ...
    while (!valid && j < poly->size() && j >= 0) {
      ...
      if (ixe != ix) {
        ...
        float dist =
            sqrtf((float)((iy - iye) * (iy - iye) + (ix - ixe) * (ix - ixe)));
    ...
    if (valid) {
      ...
      if (test.bx > xb && test.ex < xe && test.by > yb && test.ey < ye) {
        ...
        if (check) {
          ...
          for (int k = div2; k < poly->size(); k++)
            poly2.append(QPoint(poly->point(k)));
...
void contour::get_shifted_image_pos_fract(float fx, float fy, int *id1,
                                          int *id2) {
...
box::box(int ix, int iy, QString txt, float ang,
