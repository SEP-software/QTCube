...
three_face::three_face(QString view, std::shared_ptr<QFontMetrics> f_m,
                       std::shared_ptr<pick_draw> _pks,
                       std::shared_ptr<draw_other> dr,
...
void three_face::viewit(QPainter *painter, QPen *pen,
                        std::shared_ptr<slice> fact,
  ...
  slices[0]->set_draw_params(fm, pos, 0, 1, 0, front_x_b, front_x_e, front_y_b,
                             front_y_e, draws->draw_pos, false,
                             draws->draw_axis1, draws->draw_axis2, false, 0.,
                             0., false, false);

  slices[1]->set_draw_params(
      fm, pos, 0, 2, 0, side_x_b, side_x_e, side_y_b, side_y_e, draws->draw_pos,
      false, draws->draw_axis3, false, false, 0., 0., false, false);
  slices[2]->set_draw_params(fm, pos, 2, 1, 0, top_x_b, top_x_e, top_y_b,
                             top_y_e, draws->draw_pos, false, false,
                             draws->draw_axis3, false, 0., 0., true, false);
