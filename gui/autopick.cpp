...
void autopick::pick_line(std::shared_ptr<view> myv, float *fpos,
  ...
  if (myp->size() > 1) {
    ...
    if (correlate) {
      std::shared_ptr<float_2d> ar = extract_dat(
...
std::shared_ptr<float_2d> autopick::correlate_it(std::shared_ptr<float_2d> ar,
                                                 int ncor,
                                                 std::vector<int> close) {
  axis ax[2], ain1, ain2;
  ;
  ain1 = ar->getAxis(1);
  ax[1] = ain2 = ar->getAxis(2);
  ax[0] = ain1;
  ...
  for (int i2 = 0; i2 < ain2.n; i2++) {
    ...
    if (i2 == iless || iloc == (int)close.size() - 1) {  // Right on pick
      for (int i = 0; i < ain1.n; i++)
        master[i] = ar->vals[i + ain1.n * close[iloc]];

    } else {
      f = ((float)(i2 - iless) / ((float)(close[iloc + 1] - close[iloc])));
      for (int i = 0; i < ain1.n; i++) {
        master[i] = ar->vals[i + ain1.n * close[iloc]] * (1. - f) +
                    f * ar->vals[i + ain1.n * close[iloc + 1]];
      }
    }
...
std::shared_ptr<float_2d> autopick::extract_dat(
    std::shared_ptr<orient_cube> pos, std::shared_ptr<dataset> cur_dat,
    int isingle, int isort, std::shared_ptr<float_1d> line, int ns, int f2) {
  int n1 = 1, n2 = 1;
  unsigned char *buf;
  ...
  for (int i2 = 0; i2 < axes[1].n; i2++) {
    ...
    for (int i1 = 0; i1 < ns * 2 + 1; i1++) {
      i = ishift - ns + i1;
      if (i < 0)
        dat->vals[i1 + i2 * (2 * ns + 1)] = 0.;
      else if (i > n1 - 2)
        dat->vals[i1 + i2 * (ns * 2 + 1)] = 0.;
      else
        dat->vals[i1 + i2 * (ns * 2 + 1)] =
            (1. - f) * buf[i + (i2 + b2 + f2) * n1] +
