...
using namespace SEP;
void move_semblance::set_moveout_basics(std::shared_ptr<hypercube> g,
                                        std::shared_ptr<Qdataset> dat, int it,
                                        int ioff, int imes,
...
void move_semblance::read_block_float(std::vector<int> &nw,
...
void move_semblance::read_block_byte(std::vector<int> &nw, std::vector<int> &fw,
                                     unsigned char *buf) {
  if (nw[i_t] != grid->getAxis(i_t + 1).n ||
      nw[i_mes] != grid->getAxis(i_mes + 1).n)
    par->error("Internal error in measure");
