...
std::vector<QString> view::key_release_view(QKeyEvent *e,
...
std::vector<QString> view::key_press_view(QKeyEvent *e,
                                          std::shared_ptr<orient_cube> pos) {
  if (e == 0 || pos == 0) {
    ;
  }
...
void view::delete_slices() { slices.clear(); }
void view::set_title_pos(QPainter *painter, std::shared_ptr<dataset> dat) {
  ...
  get_pcts(&py, &px);
  int pw = fm->width(QString::fromStdString(dat->get_name()));
  int ph = fm->height();
...
void view::draw_bar(int xshift, QPainter *painter, QPen *pen,
                    std::shared_ptr<raster> fact, unsigned char *conv,
                    bool draw_title) {
  QString aa, title = QString::fromStdString(cur_dat->get_values());
  ...
  for (int i = 0; i < 256; i++, ii++) {
    uc[ii] = (unsigned int)conv[i];
  }
  ...
  for (int i = 0; i < (int)tics.size(); i++) {
    ...
    painter->drawLine(xb, ey - ph * 3 - yshift, xb,
                      ey - ph * 3 - yshift + tic_len);
  ...
  QPolygon pointarray;
  pointarray.putPoints(0, 5, bbar, ey - 5 * ph - yshift, bbar,
                       ey - 3 * ph - yshift, ebar, ey - 3 * ph - yshift, ebar,
                       ey - 5 * ph - yshift, bbar, ey - 5 * ph - yshift);
...
std::vector<QString> view::do_noth(float x, float y,
                                   std::shared_ptr<orient_cube> pos) {
  if (x == 0. && y == 0. && pos == 0) {
    ;
  }
...
std::vector<QString> view::r_mouse_d_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
  ...
  if (com[1].contains("pass") > 0)
    return r_mouse_d_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::r_mouse_m_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  ...
  if (com[1].contains("pass") > 0)
    return r_mouse_m_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::r_mouse_r_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
  ...
  if (!found) mode->r_mouse_d(slices, -1, ix, iy, pos, &com, draw_o);
  if (com[1].contains("pass") > 0)
    return r_mouse_r_e(ix, iy, pos, mode, draw_o);
  return com;
}

std::vector<QString> view::m_mouse_d_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
  ...
  if (com[1].contains("pass") > 0)
    return m_mouse_d_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::m_mouse_m_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
  ...
  if (com[1].contains("pass") > 0)
    return m_mouse_m_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::m_mouse_r_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  ...
  if (com[1].contains("pass") > 0)
    return m_mouse_r_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::l_mouse_d_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
...
std::vector<QString> view::l_mouse_m_view(float x, float y,
  ...
  if (com[1].contains("pass") > 0)
    return l_mouse_m_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::l_mouse_r_view(float x, float y,
                                          std::shared_ptr<orient_cube> pos,
                                          std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
  ...
  if (com[1].contains("pass") > 0)
    return l_mouse_r_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::l_mouse_double_view(
    float x, float y, std::shared_ptr<orient_cube> pos,
    std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
  this->convert_to_local_loc(x, y, &ix, &iy);
  ...
  if (com[1].contains("pass") > 0)
    return l_mouse_double_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::m_mouse_double_view(
    float x, float y, std::shared_ptr<orient_cube> pos,
    std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  bool found = false;
  ...
  if (com[1].contains("pass") > 0)
    return m_mouse_double_e(ix, iy, pos, mode, draw_o);
  return com;
}
std::vector<QString> view::r_mouse_double_view(
    float x, float y, std::shared_ptr<orient_cube> pos,
    std::shared_ptr<mouse_func> mode) {
  int ix, iy;
  ...
  if (com[1].contains("pass") > 0)
    return r_mouse_double_e(ix, iy, pos, mode, draw_o);
  return com;
}
void view::render_it(QPainter *painter, QPen *pen, std::shared_ptr<slice> fact,
                     std::shared_ptr<dataset> dat,
                     std::shared_ptr<orient_cube> pos, bool overlay,
                     std::shared_ptr<draw_what> draws) {
  for (int i = 0; i < (int)slices.size(); i++) {
    slices[i]->draw_slice(painter, dat, pen, pos, overlay, draws->draw_grid);
    // if(draws->draw_picks && cur_dat->pickable ) this->pg->draw(painter,
    // slices[i],
    // dat);
    if (draws->draw_picks && cur_dat->pickable && !overlay)
      this->pk->draw(painter, slices[i]);
  }
  ...
  if (draws->draw_bar && !overlay)
    draw_bar(4 * bound_l, painter, pen, std::static_pointer_cast<raster>(fact),
             dat->conv, draws->draw_title);
  if (draws->draw_title && !overlay)
    draw_title(0, painter, pen, QString::fromStdString(dat->get_name()));
}
