...
void slice::draw_axes(QPainter *painter, std::shared_ptr<orient_cube> pos,
  ...
  if (reverse1 && display1 >= 0) {
    ...
    for (int i = 0; i < (int)tic_loc1.size(); i++) junk.push_back(tic_loc1[i]);
    for (int i = 0; i < (int)junk.size(); i++)
      tic_loc1[tic_loc1.size() - 1 - i] = junk[i];
    junk.clear();
    for (int i = 0; i < (int)tic_val1.size(); i++) junk.push_back(tic_val1[i]);
    for (int i = 0; i < (int)junk.size(); i++)
  ...
  if (reverse2 && display2 >= 0) {
    std::vector<float> junk;
    for (int i = 0; i < (int)tic_loc2.size(); i++) junk.push_back(tic_loc2[i]);
    for (int i = 0; i < (int)junk.size(); i++)
      tic_loc2[tic_loc2.size() - 1 - i] = junk[i];
    junk.clear();
    for (int i = 0; i < (int)tic_val2.size(); i++) junk.push_back(tic_val2[i]);
    for (int i = 0; i < (int)junk.size(); i++)
