...
class anno {
 public:
  ...
  virtual void draw(QPainter *painter, std::shared_ptr<my_fonts> fonts,
                    std::shared_ptr<my_colors> col, int bx, int ex, int by,
                    int ey) {
...
class anno_text : public anno {
 public:
  anno_text() { ; }
  anno_text(float px, float py, QString c, QString t, QString f, int th) {
    txt = t;
    font = f;
    pctx = px;
    pcty = py;
...
class anno_line : public anno {
 public:
  anno_line() { ; }
  anno_line(float px, float py, QString c, int t, float ex, float ey) {
    thick = t;
    pctx = px;
    pcty = py;
    col = c;
...
class anno_arrow : public anno {
 public:
  anno_arrow() { ; }
  anno_arrow(float px, float py, QString c, int t, float ex, float ey) {
    thick = t;
    pctx = px;
    pcty = py;
    col = c;
...
class anno_ellipse : public anno {
 public:
  anno_ellipse() { ; }
  anno_ellipse(float px, float py, QString c, int t, float ex, float ey) {
    thick = t;
    pctx = px;
    pcty = py;
    col = c;
...
class anno_box : public anno {
 public:
  anno_box() { ; }
  anno_box(float px, float py, QString c, int t, float ex, float ey) {
    thick = t;
    pctx = px;
    pcty = py;
    col = c;
...
class annotate {
 public:
  ...
  void set_cur_thick(QString col) {
    cur_thick = col.toInt();
    fprintf(stderr, "setting thickness %d %s \n", cur_thick,
            col.toLatin1().constData());
  }
