  ...
  for (iv = 0; iv < nv; ++iv) {
    ...
    for (itout = 0; itout < nt; ++itout) {
      ...
      for (int is = ismin; is < ismax; ++is) {
        nsum += num[iv * nt + is] * num[iv * nt + is];
        dsum += nnz[iv * nt + is] * den[iv * nt + is];
      }
...
void nmo_semblance::update_vel(int *iloc) {
  ...
  interval_pick::form_interval_vel(dumb, grid, ds, pks, col1, col2, i_mes, i_t,
                                   i_off);
}
