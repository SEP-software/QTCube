...
class panels : public QObject {
 ...
 public:
  panels(std::shared_ptr<SEP::genericIO> io, std::shared_ptr<pick_draw> p,
         std::shared_ptr<maps> my_maps, std::shared_ptr<orients> my_o);
 signals:
 ...
 public:
  ...
  void update_it(std::shared_ptr<panel> active) {
    if (locked)
      update_all();
    else
      update_only(active);
  ...
  void update_only(std::shared_ptr<panel> active) {
    active->set_update_it(true);
  }
